...
def _init_db():
    ...
    # Auto-transfer schema upgrades for dynamic prompt settings
    for stmt in (
...
def parse_master_caption_bundle(album_msgs, target_tags):
    ...
    if master_caption:
        ...
        if lines:
            ...
            top_matched_tags = [t for t in first_line_tags if t in target_tags_lower]
        ...
        for i, am in enumerate(album_msgs, start=1):
            ...
            if i in track_data:
                ...
                am._custom_album = tags[0]     # Backward compatibility for VK
                am._custom_albums = tags       # Multi-tag support for Telegram Topics
        ...
        for i, am in enumerate(album_msgs, start=1):
            ...
            if matched_tags:
                ...
                am._custom_caption = txt or f"Imported (#{matched_tags[0]})"
                am._relative_idx = i
                matched_results.append(am)
...
async def build_topic_dedupe_cache(master_forum_id, topic_id):
    """Scans the destination forum topic to build a set of already copied file_unique_ids."""
    cache = set()
...
async def tg_execute_message_copy(src_chat_id, m, master_forum_id, topic_id, tag, delete_original, topic_cache=None):
    ...
    if m.media_group_id:
        ...
        # Check stateless cache to see if the group was already migrated
        unseen_msgs = []
        for gm in group_msgs:
            f_id = gm.video.file_unique_id if gm.video else (gm.document.file_unique_id if gm.document else None)
        ...
        if not unseen_msgs: # Entire media group already exists in the destination
            return False
...
async def run_custom_transfer(user_chat_id, state, mode, custom_tags):
    ...
    # ---------------------------------------------------------
    # OPTIMIZED TAGS SEARCH: Fast API search instead of full scan
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # ALL MODE: Full scan with live progress updates
    # ---------------------------------------------------------
    elif mode == "ALL":
        scanned_count = 0
...
async def live_monitor_handler(client, message):
    ...
    # LIVE AUTOTRANSFER HANDLING (FORUM TOPICS)
