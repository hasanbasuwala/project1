...
# FIX: Pyrogram handles Telegram's FLOOD_WAIT_X for upload.GetFile internally by
# sleeping — that's what "[user_session] Waiting for N seconds..." log lines are.
# It's not a crash, it's the client obeying Telegram's rate limit. But if we don't
# react to it, the download engine keeps hammering at the same concurrency and the
# waits just keep escalating over a big batch. These settings let us detect that
# pattern and automatically throttle concurrency down for a while.
FLOOD_WAIT_WINDOW = 120          # seconds - rolling window to count flood-wait events
FLOOD_WAIT_THRESHOLD = 4         # this many flood-waits within the window trips throttle-down
FLOOD_THROTTLE_SECONDS = 120     # how long to force reduced concurrency once tripped

# ============================================================
# TELEGRAM TRANSFER ENGINE CONFIG (NEW)
# ============================================================
# Pacing between Telegram-to-Telegram copy operations (copy_message / copy_media_group,
# group creation, member/admin promotion, username assignment). Telegram flood-limits
# these server-side "content management" style calls much more aggressively than plain
# messaging, so we deliberately stay well under the threshold rather than relying only
# on catching FloodWait after the fact.
...
# ============================================================
# Pyrogram logs a line like:
#   [user_session] Waiting for 9 seconds before continuing (required by "upload.GetFile")
# whenever Telegram returns FLOOD_WAIT_X for a file-chunk request, and then sleeps
# internally before retrying. It's not an error, but repeated/escalating waits mean
# our download concurrency is outrunning what this account is allowed to request.
# We hook Pyrogram's logger to notice this pattern and automatically throttle
# concurrency down for a while rather than continuing to trigger longer waits.
...
# FIX: fast reactive trigger for generic socket-level failures ("socket.send() raised
# exception.", broken pipe, connection reset). This message doesn't identify which
# client/session it came from, so when it storms we can't tell if it's user_app,
# bot_app, or both — the safe move is to reconnect both rather than guess wrong and
# leave one of them (e.g. the bot session driving your commands/dashboard) still dead.
# The connection_watchdog below also catches this within ~2 minutes even without this
# handler, but reacting to the log directly is much faster when it's actively storming.
...
# FIX: reconnect state is now PER CLIENT (user_app vs bot_app), since either one can
# independently lose its connection. The earlier version only ever restarted user_app,
# which meant a dead bot_app connection (dashboard/commands unresponsive) had no
# recovery path at all — exactly the "no bot commands work" symptom.
last_reconnect_time = {"user": 0.0, "bot": 0.0}
reconnect_locks = {"user": asyncio.Lock(), "bot": asyncio.Lock()}

# FIX: proactive health check settings. Everything above (record_network_error) only
# ever fires from WITHIN the download pipeline — once a batch finishes and nothing is
# downloading, nothing calls it anymore, even if the connection is actually dead. This
# watchdog runs continuously regardless of activity level and self-heals idle sessions.
CONNECTION_HEALTHCHECK_INTERVAL = 60      # seconds between health checks
CONNECTION_HEALTHCHECK_TIMEOUT = 15       # seconds to wait for a response before failing
CONNECTION_HEALTHCHECK_FAIL_THRESHOLD = 2 # consecutive failures before forcing a reconnect
...
async def record_network_error(exc):
    ...
    transfers for a cooldown and kick off a reconnect of the user session (this
    always originates from download/message-fetch code, so it's always user_app)."""
...
async def reconnect_client(client, label):
    """Force-restart a Pyrogram client (either user_app or bot_app). Guarded per-label
    so only one reconnect of a given client runs at a time, and we don't reconnect more
    than once per RECONNECT_MIN_INTERVAL seconds even if multiple triggers fire close
    together. Existing @client.on_message/@client.on_callback_query handlers stay
    registered across a stop/start cycle — they're attached to the client object, not
    the underlying connection — so no re-registration is needed after reconnecting."""
...
async def connection_watchdog():
    """Runs forever, independent of download/upload activity, so a connection that
    dies during an idle period (e.g. right after a batch finishes, when nothing else
    would notice) still gets detected and recovered. Pings each client with a cheap
    get_me() call; two consecutive failures on either one triggers a reconnect of
    just that client."""
    while True:
...
def _init_db():
    ...
    # ============================================================
    # NEW: TELEGRAM TRANSFER ENGINE TABLES
    # ============================================================
    # Per-tag destination config. dest_type is 'vk' (existing pipeline) or 'tg_copy'
    # (new Telegram-to-Telegram transfer engine). Set once via the DM "#tag" wizard,
    # then reused by /transfer, /copy, /autotransfer and the VK monitor flow alike.
    conn.execute("""
    ...
    # Dedupe ledger so /transfer, /copy and /autotransfer never re-copy the same
    # source message into the same destination group twice, even across restarts.
    conn.execute("""
        CREATE TABLE IF NOT EXISTS transferred_messages (
            ...
            dest_chat_id INTEGER,
            transferred_at REAL,
            PRIMARY KEY (chat_id, msg_id, dest_chat_id)
        )
    """)

    # Continuous /autotransfer monitors — one row per source group being watched.
    conn.execute("""
...
# ============================================================
# NEW: TELEGRAM TRANSFER ENGINE (/transfer, /copy, /autotransfer)
# ============================================================
# Everything here moves media Telegram -> Telegram using server-side copy_message /
# copy_media_group calls. No download, no ffprobe/ffmpeg, no re-encode is needed —
# Telegram does the byte-moving on its own servers. The only thing we own is: pacing
# (to avoid flood limits), dedupe bookkeeping, and destination-group lifecycle.

async def _flood_safe(coro_fn, *args, **kwargs):
    """Call a Pyrogram coroutine, transparently handling FloodWait by sleeping and
    retrying, so a burst of copy/create/promote/username calls during a transfer run
    never crashes the worker — it just paces itself to what Telegram allows."""
    attempts = 0
    while True:
        try:
            ...
            console.print(f"[bold red]🚦 FloodWait ({wait_s}s) on {getattr(coro_fn, '__name__', 'call')}, pausing...[/bold red]")
            await asyncio.sleep(wait_s)
            if attempts >= TRANSFER_MAX_RETRIES:
                raise

async def get_tag_destination(tag):
    """Returns (dest_type, visibility, username, dest_chat_id, dest_title) or None."""
    return await db_execute(
        "SELECT dest_type, visibility, username, dest_chat_id, dest_title FROM tag_destinations WHERE tag=?",
        (tag,), fetch="one"
    )

async def set_tag_destination(tag, dest_type, visibility, username=None, dest_chat_id=None, dest_title=None):
    await db_execute(
        """INSERT INTO tag_destinations (tag, dest_type, visibility, username, dest_chat_id, dest_title, created_at)
           VALUES (?,?,?,?,?,?,?)
           ON CONFLICT(tag) DO UPDATE SET dest_type=excluded.dest_type, visibility=excluded.visibility,
               username=COALESCE(excluded.username, tag_destinations.username),
               dest_chat_id=COALESCE(excluded.dest_chat_id, tag_destinations.dest_chat_id),
               dest_title=COALESCE(excluded.dest_title, tag_destinations.dest_title)""",
        (tag, dest_type, visibility, username, dest_chat_id, dest_title, time.time())
    )

async def is_msg_transferred(chat_id, msg_id, dest_chat_id):
    row = await db_execute(
        "SELECT 1 FROM transferred_messages WHERE chat_id=? AND msg_id=? AND dest_chat_id=?",
        (chat_id, msg_id, dest_chat_id), fetch="one"
    )
    return bool(row)

async def mark_msg_transferred(chat_id, msg_id, tag, dest_chat_id):
    await db_execute(
        "INSERT OR IGNORE INTO transferred_messages (chat_id, msg_id, tag, dest_chat_id, transferred_at) VALUES (?,?,?,?,?)",
        (chat_id, msg_id, tag, dest_chat_id, time.time())
    )

async def create_transfer_group(tag, visibility, username, owner_user_id):
    """Creates a new private/public supergroup for a tag, adds+promotes the requesting
    user to full admin, and (if public) assigns the chosen username. Returns
    (dest_chat_id, dest_title)."""
    title = tag.replace("#", "").strip() or "transfer"
    chat = await _flood_safe(user_app.create_supergroup, title, f"Auto-transfer group for {tag}")
    dest_chat_id = chat.id
    await asyncio.sleep(TRANSFER_STAGGER_SECONDS)

    if owner_user_id:
        try:
            await _flood_safe(user_app.add_chat_members, dest_chat_id, owner_user_id)
            await asyncio.sleep(TRANSFER_STAGGER_SECONDS)
        except Exception as e:
            console.print(f"[yellow]⚠️ Couldn't add owner to new group '{title}': {e}[/yellow]")
        try:
            await _flood_safe(
                user_app.promote_chat_member,
                dest_chat_id, owner_user_id,
            ...
            console.print(f"[yellow]⚠️ Couldn't promote owner in new group '{title}': {e}[/yellow]")

    if visibility == "public" and username:
        try:
            await _flood_safe(user_app.set_chat_username, dest_chat_id, username)
            await asyncio.sleep(TRANSFER_STAGGER_SECONDS)
        except Exception as e:
            console.print(f"[yellow]⚠️ Couldn't set public username '@{username}' for '{title}': {e}[/yellow]")

    await set_tag_destination(tag, "tg_copy", visibility, username, dest_chat_id, title)
    console.print(f"[bold green]✅ Created transfer group '{title}' ({dest_chat_id}) for {tag}[/bold green]")
    return dest_chat_id, title

async def get_or_create_transfer_group(tag, owner_user_id):
    """Reuses the tag's destination group if one already exists, otherwise creates one
    (defaulting to private with no username if the tag was never explicitly configured
    via the DM wizard — e.g. a tag matched inside a group run of /transfer or /copy)."""
    dest = await get_tag_destination(tag)
    if dest and dest[3]:  # dest_chat_id already set
        return dest[3], dest[4]
    visibility = dest[1] if dest else "private"
    username = dest[2] if dest else None
    return await create_transfer_group(tag, visibility, username, owner_user_id)

async def copy_matched_message(src_chat_id, m, dest_chat_id, tag, delete_original):
    """Copies a single matched message (or its whole album, if it belongs to one) from
    src_chat_id to dest_chat_id. Paced for flood-limits, dedupe-checked, and optionally
    deletes the source afterward. Returns True if a copy actually happened."""
    if await is_msg_transferred(src_chat_id, m.id, dest_chat_id):
        return False
    ...
    if m.media_group_id:
        ...
        await _flood_safe(user_app.copy_media_group, dest_chat_id, src_chat_id, m.id)
        for mid in ids:
            await mark_msg_transferred(src_chat_id, mid, tag, dest_chat_id)
        if delete_original:
            try:
                await user_app.delete_messages(src_chat_id, ids)
            except Exception as e:
                console.print(f"[yellow]⚠️ Couldn't delete originals for album starting at {m.id}: {e}[/yellow]")
    else:
        await _flood_safe(user_app.copy_message, dest_chat_id, src_chat_id, m.id)
        await mark_msg_transferred(src_chat_id, m.id, tag, dest_chat_id)
        if delete_original:
            try:
                await user_app.delete_messages(src_chat_id, m.id)
            except Exception as e:
                console.print(f"[yellow]⚠️ Couldn't delete original {m.id}: {e}[/yellow]")

    await asyncio.sleep(TRANSFER_STAGGER_SECONDS)
    return True

async def scan_chat_for_all_tags(chat_id, limit=None):
    """Scans a chat's history once and groups every message matching a currently
    registered tag (selected_tags) into {tag: [messages...]}, using the same
    master-caption-bundle parser as Selective Sync so numbered album captions behave
    identically here."""
    tags_rows = await db_execute("SELECT tag FROM selected_tags", fetch="all")
    target_tags = [r[0] for r in tags_rows] if tags_rows else []
    if not target_tags:
        return {}
    ...
    async for msg in user_app.get_chat_history(chat_id, **kwargs):
        ...
        matched = parse_master_caption_bundle(album_msgs, target_tags)
        for m in matched:
            tag_clean = getattr(m, '_custom_album', None)
            if not tag_clean:
                continue
            tag = f"#{tag_clean}"
            results.setdefault(tag, []).append(m)

    return results

async def run_tag_by_tag_transfer(chat_id, owner_user_id, delete_originals, status_cb=None):
    """The core engine behind /transfer, /copy, and the historical backfill half of
    /autotransfer. Scans the chat ONCE, groups matches by tag, then processes ONE TAG
    AT A TIME in full: resolve/create that tag's destination group, copy every matched
    message for that tag (paced for flood limits), THEN move to the next tag. Tags
    already configured for VK ('vk' dest_type) are skipped here — they stay on the
    existing VK monitor/queue pipeline."""
    tag_matches = await scan_chat_for_all_tags(chat_id)
    if not tag_matches:
        return {}

    summary = {}
    for tag, msgs in tag_matches.items():
        dest = await get_tag_destination(tag)
        if dest and dest[0] == "vk":
            continue

        dest_chat_id, dest_title = await get_or_create_transfer_group(tag, owner_user_id)
        if status_cb:
            await status_cb(tag, dest_title, len(msgs))

        copied = 0
        for m in sorted(msgs, key=lambda x: x.id):
            try:
                if await copy_matched_message(chat_id, m, dest_chat_id, tag, delete_originals):
                    copied += 1
            except Exception as e:
                console.print(f"[red]⚠️ Transfer failed for msg {m.id} ({tag}): {e}[/red]")

        summary[tag] = {"dest_title": dest_title, "dest_chat_id": dest_chat_id, "copied": copied, "found": len(msgs)}
...
async def live_monitor_handler(client, message):
    ...
    # ============================================================
    # NEW: LIVE /autotransfer HANDLING
    # ============================================================
    # Mirrors the always/selected blocks above, but copies Telegram -> Telegram instead
    # of queueing a VK download. Tags configured for VK ('vk' dest_type) are skipped
    # here so a tag never gets double-handled by both pipelines.
    autotransfer_row = await db_execute(
        "SELECT status, delete_originals, owner_user_id FROM autotransfer_monitors WHERE chat_id=?",
        (chat_id,), fetch="one"
    )
    if autotransfer_row and autotransfer_row[0] == "ACTIVE":
        del_orig = bool(autotransfer_row[1])
        owner_uid = autotransfer_row[2]
        at_tags_rows = await db_execute("SELECT tag FROM selected_tags", fetch="all")
        at_target_tags = [r[0] for r in at_tags_rows] if at_tags_rows else []

        if at_target_tags:
            at_album_msgs = [message]
            if message.media_group_id:
                try:
                    at_album_msgs = await user_app.get_media_group(chat_id, message.id)
                    at_album_msgs = sorted(at_album_msgs, key=lambda x: x.id)
                except Exception:
                    at_album_msgs = [message]

            at_matched = parse_master_caption_bundle(at_album_msgs, at_target_tags)
            for m in at_matched:
                tag_clean = getattr(m, '_custom_album', None)
                if not tag_clean:
                    continue
                ...
                if dest and dest[0] == "vk":
                    continue
                try:
                    dest_chat_id, _ = await get_or_create_transfer_group(tag, owner_uid)
                    await copy_matched_message(chat_id, m, dest_chat_id, tag, del_orig)
                except Exception as e:
...
def get_part_count(file_size):
    # FIX: if Telegram has been flood-limiting upload.GetFile recently, force
    # sequential single-part downloads for a while — splitting one file into several
    # parallel chunk streams is exactly what multiplies request rate and triggers
    # (and escalates) FLOOD_WAIT_X in the first place.
            ...
            try:
                ...
                with open(part_file, "wb") as f:
                    ...
                    async for chunk in client.stream_media(file_id, limit=chunk_limit, offset=chunk_offset):
                        ...
                        # FIX: while the flood-wait throttle is active, add a small
                        # pause between chunk requests so we're not requesting the next
                        # chunk the instant the previous one lands — this is the actual
                        # request-rate reduction, on top of forcing single-part downloads
                        # above. Costs nothing when Telegram isn't rate-limiting us.
                        if time.time() < reduced_parallelism_until:
...
# ============================================================
# NEW: /transfer, /copy, /autotransfer, /reset COMMANDS
# ============================================================

async def _resolve_target_chat(message, command_name):
    """Shared resolution logic for /transfer, /copy, /autotransfer: if run directly
    inside a group/supergroup, that chat IS the source. If run in DM, the next word
    after the command must be a group id or @username to resolve."""
...
@bot_app.on_message(filters.command(["transfer", "copy"]))
async def transfer_copy_cmd(client, message):
    """/transfer copies matched videos to their tag's Telegram destination group and
    deletes the originals. /copy does the same but leaves originals untouched. Both
    act on ALL tags currently registered in Selective Sync when run directly inside a
    group; in DM they take a target group as an argument. Tags configured for VK are
    skipped (they stay on the existing VK pipeline)."""
    ...
    status_msg = await message.reply_text(
        f"🔎 Scanning **{target_title}** for all registered tags ({'transfer' if delete_originals else 'copy'})...",
        parse_mode=ParseMode.MARKDOWN
    )

    async def progress(tag, dest_title, count):
        try:
            await status_msg.edit_text(f"📦 Processing {tag} → **{dest_title}** ({count} matched)...", parse_mode=ParseMode.MARKDOWN)
        except Exception:
            pass

    summary = await run_tag_by_tag_transfer(target_chat_id, owner_user_id, delete_originals, status_cb=progress)

    if not summary:
        return await status_msg.edit_text("ℹ️ No registered tags matched anything in this chat (or all matched tags are VK-bound).")

    lines = [f"✅ **{'Transfer' if delete_originals else 'Copy'} Complete: {target_title}**", "━━━━━━━━━━━━━━━━━━"]
    for tag, info in summary.items():
        lines.append(f"• {tag} → **{info['dest_title']}**: {info['copied']}/{info['found']} copied")
    await status_msg.edit_text("\n".join(lines), parse_mode=ParseMode.MARKDOWN)

@bot_app.on_message(filters.command("autotransfer"))
async def autotransfer_cmd(client, message):
    """Sets up continuous Telegram-to-Telegram auto-transfer for a group: on setup, runs
    a ONE-TIME historical backfill (tag-by-tag: scan once, collect all matched tags,
    then create/reuse each tag's group and transfer all its matches before moving to
    the next tag), then keeps monitoring live for new matching messages going forward."""
    owner_user_id = message.from_user.id if message.from_user else None
    target_chat_id, target_title = await _resolve_target_chat(message, "autotransfer")
    if target_chat_id is None:
        return

    kbd = InlineKeyboardMarkup([[
        InlineKeyboardButton("🗑️ Delete originals", callback_data=f"atr_setup_del_{target_chat_id}_{owner_user_id or 0}"),
        InlineKeyboardButton("📋 Keep originals", callback_data=f"atr_setup_keep_{target_chat_id}_{owner_user_id or 0}")
    ]])
    await message.reply_text(
        f"📡 **Auto-Transfer Setup: {target_title}**\nShould originals be deleted after copying, or kept?",
...
async def reset_cmd(client, message):
    ...
    await message.reply_text(
        "⚠️ **This will permanently delete ALL jobs, playlists, monitors, tag destinations, "
...
@bot_app.on_message(filters.private & filters.text & ~filters.command(["start", "help", "refresh", "monitor", "monitoralways", "monitorselected", "transfer", "copy", "autotransfer", "reset"]))
async def handle_user_input(client, message):
    ...
    # ============================================================
    # NEW: destination-setup wizard for a freshly DM'd "#tag"
    # ============================================================
    if state.get('awaiting_dest_username'):
        username = message.text.strip().lstrip("@")
        st = user_states.get(chat_id, {})
        st['pending_username'] = username
        st.pop('awaiting_dest_username', None)
        user_states[chat_id] = st
        kbd = InlineKeyboardMarkup([[
            InlineKeyboardButton("📤 Telegram Copy", callback_data="destmethod_tg"),
            InlineKeyboardButton("🎞️ VK Download", callback_data="destmethod_vk")
        ]])
        await message.reply_text(f"🌐 Username set to @{username}. Sync method for this tag?", reply_markup=kbd)
        return
    ...
    if not state.get('awaiting_group'):
        ...
        existing_dest = await get_tag_destination(search_query)
        if existing_dest:
            # Tag already has a destination configured — go straight to the legacy
            # group-id-scan flow, same as before.
        ...
        user_states[chat_id] = {'pending_tag_setup': search_query}
        kbd = InlineKeyboardMarkup([[
            InlineKeyboardButton("🔒 Private", callback_data="destvis_private"),
            InlineKeyboardButton("🌐 Public", callback_data="destvis_public")
        ]])
        return await message.reply_text(
            f"⚙️ New tag *{search_query}* — should its destination be private or public?",
            reply_markup=kbd, parse_mode=ParseMode.MARKDOWN
        )
...
async def handle_buttons(client, callback):
    ...
    # ============================================================
    # NEW: tag destination wizard callbacks
    # ============================================================
    elif data in ("destvis_private", "destvis_public"):
        st = user_states.get(chat_id, {})
        tag = st.get('pending_tag_setup')
        if not tag:
            return await callback.answer("Session expired.", show_alert=True)
        visibility = "private" if data == "destvis_private" else "public"
        user_states[chat_id] = {'pending_tag_setup': tag, 'pending_visibility': visibility}
        if visibility == "public":
            user_states[chat_id]['awaiting_dest_username'] = True
            await callback.message.edit_text(
                f"🌐 Public selected for *{tag}*. Send the desired @username (letters/digits/underscore only):",
                parse_mode=ParseMode.MARKDOWN
            )
            return await callback.answer()
        else:
            ...
            await callback.message.edit_text(f"🔒 Private selected for *{tag}*. Sync method?", reply_markup=kbd, parse_mode=ParseMode.MARKDOWN)
            return await callback.answer()

    elif data in ("destmethod_tg", "destmethod_vk"):
        st = user_states.get(chat_id, {})
        tag = st.get('pending_tag_setup')
        visibility = st.get('pending_visibility', 'private')
        username = st.get('pending_username')
        if not tag:
            return await callback.answer("Session expired.", show_alert=True)

        if data == "destmethod_vk":
            await set_tag_destination(tag, "vk", visibility, username, None, None)
            await callback.message.edit_text(
                f"✅ *{tag}* configured for **VK Download**.\nNow send the **Group Chat ID** to search.",
                parse_mode=ParseMode.MARKDOWN
            )
        else:
            await set_tag_destination(tag, "tg_copy", visibility, username, None, None)
            await callback.message.edit_text(
                f"✅ *{tag}* configured for **Telegram Copy** ({visibility}).\nNow send the **Group Chat ID** to search.",
                parse_mode=ParseMode.MARKDOWN
            )

        user_states[chat_id] = {'query': tag, 'awaiting_group': True}
        return await callback.answer()

    # ============================================================
    # NEW: /autotransfer setup + pause/resume/stop callbacks
    # ============================================================
    elif data.startswith("atr_setup_del_") or data.startswith("atr_setup_keep_"):
        delete_originals = data.startswith("atr_setup_del_")
        prefix = "atr_setup_del_" if delete_originals else "atr_setup_keep_"
        rest = data[len(prefix):]
        c_id_str, owner_str = rest.rsplit("_", 1)
        c_id = int(c_id_str)
        owner_user_id = int(owner_str) if owner_str not in ("0", "None") else None

        try:
            chat_obj = await user_app.get_chat(c_id)
            title = chat_obj.title or str(c_id)
        except Exception:
            title = str(c_id)

        await db_execute(
            """INSERT INTO autotransfer_monitors (chat_id, chat_title, status, delete_originals, owner_user_id, last_msg_id, added_at)
               VALUES (?,?,'ACTIVE',?,?,0,?)
               ON CONFLICT(chat_id) DO UPDATE SET status='ACTIVE', delete_originals=excluded.delete_originals, owner_user_id=excluded.owner_user_id""",
            (c_id, title, int(delete_originals), owner_user_id, time.time())
        )

        await callback.answer("✅ Auto-Transfer active. Running historical backfill now...", show_alert=True)
        await callback.message.edit_text(f"⚙️ **Auto-Transfer Active: {title}**\nRunning tag-by-tag historical backfill in the background...")

        async def _backfill():
            summary = await run_tag_by_tag_transfer(c_id, owner_user_id, delete_originals)
            lines = [f"✅ **Auto-Transfer Backfill Complete: {title}**", "━━━━━━━━━━━━━━━━━━"]
            for tag, info in summary.items():
                lines.append(f"• {tag} → **{info['dest_title']}**: {info['copied']}/{info['found']} copied")
            if not summary:
                lines.append("_No registered tags matched anything._")
            try:
                await bot_app.send_message(callback.message.chat.id, "\n".join(lines), parse_mode=ParseMode.MARKDOWN)
            except Exception:
                pass
    ...
    # ============================================================
    # NEW: /reset confirmation callbacks
    # ============================================================
        ...
        for table in ("jobs", "playlists", "monitored_chats", "monitored_messages", "monitored_tags_meta",
                      ...
                      "tag_destinations", "transferred_messages", "autotransfer_monitors"):
        ...
        # NEW: if this tag was configured for Telegram Copy, transfer via the new
        # engine instead of building a VK playlist.
        dest = await get_tag_destination(state['query'])
        if dest and dest[0] == "tg_copy":
            await callback.answer("Transferring via Telegram copy...")
            owner_user_id = callback.from_user.id if callback.from_user else None
            dest_chat_id, dest_title = await get_or_create_transfer_group(state['query'], owner_user_id)
            copied = 0
            for m in state['found_msgs']:
                try:
                    if await copy_matched_message(m.chat.id, m, dest_chat_id, state['query'], delete_original=False):
                        copied += 1
                except Exception as e:
                    console.print(f"[red]⚠️ Transfer failed for msg {m.id}: {e}[/red]")
            user_states.pop(chat_id, None)
            await callback.message.edit_text(f"✅ Copied {copied} videos to **{dest_title}**.", parse_mode=ParseMode.MARKDOWN)
            return
...
async def main():
    ...
    await bot_app.set_bot_commands([
        ...
        BotCommand("transfer", "🚀 Transfer tagged videos (deletes originals)"),
        BotCommand("copy", "📋 Copy tagged videos (keeps originals)"),
    ...
    # NEW: Auto-transfer groups don't need a startup catch-up scan (unlike VK-bound
    # always/selected monitors above) — their live handler picks up new messages
    # going forward, and the historical backfill already ran once at setup time. We
    # just surface their current state on the dashboard so you know they survived
    # the restart.
