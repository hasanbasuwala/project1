"""
stealth_bot.py – v13.0 (Structured Monolith Edition)
───────────────────────────────────────────────────────────────
ARCHITECTURE:
  • Single-file Micro-Orchestration (Classes).
  • JobScheduler (SQLite + asyncio.Lock) replaces state.json.
  • FFmpeg Sandbox (Strict timeouts prevent zombie processes).
  • Event-Driven UI & ANSI Termux Logger pulling from SQLite.
───────────────────────────────────────────────────────────────
"""
...
logging.basicConfig(
    ...
    handlers=[RotatingFileHandler(LOG_DIR / "engine.log", maxBytes=10 * 1024 * 1024, backupCount=3, encoding="utf-8"), logging.StreamHandler()]
)
...
class JobScheduler:
    """Thread-safe SQLite wrapper replacing state.json flat files."""
        ...
        async with self.lock:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute('''INSERT INTO jobs (id, url, title, source, quality, strategy, stage, pct, last_ui_pct, retries, chat_id, tracker_id)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
                             (data['id'], data['url'], data['title'], data['source'], data.get('quality', 'best'), data.get('strategy', 'GENERIC'), 
                              Stage.QUEUED.value, 0.0, -10.0, 0, data['chat_id'], data['tracker_id']))
                
        # Physical Folder Isolation
...
class LinkClassifier:
    """Pre-processes URLs to define the exact download engine strategy."""
    @staticmethod
...
class DownloaderEngine:
    """Adaptive 6-Stage Waterfall Orchestrator."""
    ...
    async def execute(self, job_data: dict):
        ...
        if strategy == "MAGNET":
        ...
        elif strategy == "DIRECT_MP4":
            await self._run_aria(url, jid, dl_dir)
        else:
            # Stage 1: Primp HTTP Intercept
            ...
            # Stage 2 & 3: Fallback Waterfall
            await asyncio.to_thread(self._run_ytdlp, actual_url, jid, dl_dir, referer, cookie, quality)
    ...
    async def _run_aria(self, url: str, jid: str, dl_dir: Path):
        ...
        try:
            while True:
                ...
                if not chunk: break
                m = re.search(r"\((\d+)%\)", chunk.decode("utf-8", errors="ignore"))
                if m: await self.db.update_job(jid, pct=float(m.group(1)))
        finally:
            await proc.wait(); self.procs.pop(jid, None)
        if not list(dl_dir.glob("*.mp4")) and not list(dl_dir.glob("*.mkv")): raise RuntimeError("Aria2c failed.")
    ...
    def _run_ytdlp(self, url: str, jid: str, dl_dir: Path, referer: str, cookie: str, quality: str):
        def prog_hook(d):
        ...
        opts = {"outtmpl": str(dl_dir / f"{jid}.%(ext)s"), "format": fmt, "http_headers": {"Referer": referer, "User-Agent": USER_AGENT}, "progress_hooks": [prog_hook], "quiet": True}
        with yt_dlp.YoutubeDL(opts) as ydl: ydl.extract_info(url, download=True)


class EncoderEngine:
    """FFmpeg Sandbox with anti-zombie process timeouts."""
    def __init__(self, scheduler: JobScheduler):
        self.db = scheduler
    ...
    async def execute(self, job_data: dict):
        ...
        dl_file = max([f for f in dl_dir.glob("*") if f.is_file()], key=lambda p: p.stat().st_size)
        ...
        # Sandbox Remux with 15-minute strict timeout limit
...
class PipelineManager:
    def __init__(self, app: Client, db: JobScheduler):
        ...
        self.dl_engine, self.enc_engine, self.up_engine = DownloaderEngine(db), EncoderEngine(db), UploadEngine(db, app)
...
def setup_router(app: Client, db: JobScheduler, pipeline: PipelineManager):
    ...
    async def url_catcher(_, msg: Message):
        ...
        if url:
            ...
            _pending_confirmations[tok] = {"url": url, "chat_id": msg.chat.id, "title": msg.text.replace(url, "").strip() or url[:40]}
            kb = InlineKeyboardMarkup([[InlineKeyboardButton("📺 1080p", callback_data=f"conf|{tok}|1080"), InlineKeyboardButton("✖ ABORT", callback_data=f"conf|{tok}|cancel")]])
            await app.send_message(msg.chat.id, f"🔗 **VECTOR INTERCEPTED**\n`{url[:30]}...`", reply_markup=kb)

    @app.on_callback_query(filters.user(OWNER_ID))
    async def cb_router(_, cb):
        parts = cb.data.split("|"); action = parts[0]
        if action == "conf":
            tok, qual = parts[1], parts[2]
            pending = _pending_confirmations.pop(tok, None)
            if not pending: return await cb.answer("Expired.", show_alert=True)
            if qual == "cancel": return await cb.message.delete()
            
            jid, title = str(uuid.uuid4())[:8], pending['title']
            tracker = await cb.message.edit_text(f"`[ ⚡ ] ＴＡＳＫ :` `{title[:30]}`\n`[ ⚙️ ] ＳＴＡＴ :` `QUEUED`", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ CANCEL", callback_data=f"kill|{jid}")]]))
            
            await db.create_job({"id": jid, "url": pending['url'], "title": title, "source": "Direct", "quality": qual, "strategy": LinkClassifier.classify(pending['url']), "chat_id": pending['chat_id'], "tracker_id": tracker.id})
            await pipeline.dl_q.put(jid); await cb.answer("Queued.")
...
async def ui_throttle_loop(app: Client, db: JobScheduler):
    while True:
        ...
        for job in await db.get_active_jobs():
            ...
            if job['stage'] != job['last_ui_pct'] or (job['pct'] - job['last_ui_pct']) >= 10.0: # Throttled UI 
