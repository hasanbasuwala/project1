...
FEATURES (v4.1):
  ...
  • /scan #Hashtag: Scans a community wall specifically for a hashtag/name and queues them.
...
_scan_sessions = {}
_hashtag_scan_sessions = {}  # {chat_id: {'tag': str, 'stage': str, 'videos': list, 'url': str}}
...
def setup_router(app: Client, db: JobScheduler, dl_q: asyncio.Queue, enc_q: asyncio.Queue, up_q: asyncio.Queue, dl_pool: "WorkerPool", enc_pool: "WorkerPool", up_pool: "WorkerPool"):
    ...
    # ==========================================================
    # NEW FEATURE: /scan #Hashtag (WALL SEARCH BY HASHTAG/NAME)
    # ==========================================================
    ...
    async def cmd_scan_hashtag(_, msg: Message):
        ...
        if not match:
            return await msg.reply("❌ Usage: `/scan #HashtagName` (e.g., `/scan #HasanBasu`)")
        
        tag_raw = match.group(1).strip()
        _hashtag_scan_sessions[msg.chat.id] = {
            'tag': tag_raw,
            'stage': 'awaiting_link',
            'videos': [],
            'url': None
        ...
        await msg.reply(
            f"🔍 **Hashtag Scan Initiated for:** `#{tag_raw}`\n\n"
            f"Please send the **VK Community Link** you want to search across."
        )
    ...
    async def handle_hashtag_community_link(_, msg: Message):
        ...
        m = await msg.reply(f"🔍 Searching wall posts in community for `#{tag}`...")

        def search_wall_by_tag():
            vk = get_vk_api()
            comm_id = resolve_vk_community_id(vk, url)
            
            # Case-insensitive match for "HasanBasu", "Hasan Basu", or "#HasanBasu"
            spaced_tag = re.sub(r'([a-z])([A-Z])', r'\1 \2', tag)
            pattern = re.compile(rf"#?\b(?:{re.escape(tag)}|{re.escape(spaced_tag)})\b", re.IGNORECASE)
        ...
        try:
            videos = await asyncio.to_thread(search_wall_by_tag)
            session['videos'] = videos

            if not videos:
                del _hashtag_scan_sessions[msg.chat.id]
                return await m.edit(f"❌ No native videos found with `#{tag}` or `{tag}` on the wall.")

            session['stage'] = 'awaiting_confirmation'

            kb = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton(f"📥 Download & Upload ({len(videos)})", callback_data="ht_confirm_download"),
                    InlineKeyboardButton("❌ Cancel", callback_data="ht_cancel")
                ]
            ])

            await m.edit(
                f"🎯 **WALL SEARCH COMPLETE**\n`━━━━━━━━━━━━━━━━━━━━━━━━━━`\n"
                f"🏷 **Search Term:** `#{tag}`\n"
                f"📹 **Matching Videos Found:** `{len(videos)}`\n"
                f"📁 **Target Album:** `{tag}`\n\n"
                f"Do you want to download all **{len(videos)}** videos and upload them to album **#{tag}**?",
                reply_markup=kb
            )

        except Exception as e:
            del _hashtag_scan_sessions[msg.chat.id]
            await m.edit(f"❌ Error during hashtag wall search: `{e}`")
    ...
    async def handle_callbacks(_, cb: CallbackQuery):
        ...
        if cb.data == "ht_cancel":
            if cb.message.chat.id in _hashtag_scan_sessions:
                del _hashtag_scan_sessions[cb.message.chat.id]
            await cb.answer("Cancelled.")
            return await cb.message.edit("❌ Hashtag scan cancelled.")
            ...
            if not session or not session.get('videos'):
                return await cb.answer("Session expired or no videos found.", show_alert=True)

            videos = session['videos']
            album_name = session['tag']
            comm_url = session['url']

            await cb.answer("Queueing videos for download and upload...")
            await cb.message.edit("⏳ Resolving/creating album and deduplicating items...")
            ...
            try:
                target_album_id, missing_videos = await asyncio.to_thread(prepare_queue)

                if not missing_videos:
                    del _hashtag_scan_sessions[cb.message.chat.id]
                    return await cb.message.edit(f"✅ All `{len(videos)}` videos are already present in album **{album_name}**.")

                pl_id = str(uuid.uuid4())[:8]
                await db.create_playlist(pl_id, comm_url, str(target_album_id), len(missing_videos), cb.message.chat.id)

                db_items = [(str(uuid.uuid4())[:8], pl_id, item['url'], f"{item['title']}|||{item['unique_id']}") for item in missing_videos]
                await db.add_playlist_items(db_items)

                skipped = len(videos) - len(missing_videos)
                del _hashtag_scan_sessions[cb.message.chat.id]

                await cb.message.edit(
                    f"✅ **HASHTAG BATCH QUEUED**\n`━━━━━━━━━━━━━━━━━━━━━━━━━━`\n"
                    f"📁 **Album Target:** `{album_name}`\n"
                    f"🔍 **Found:** `{len(videos)}` videos\n"
                    f"⏭ **Duplicates Skipped:** `{skipped}`\n"
                    f"📥 **Queued for Processing:** `{len(missing_videos)}`"
                )
                ...
                await cb.message.edit(f"❌ Error queueing batch: `{e}`")
