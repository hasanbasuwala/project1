...
try:
    ...
    stash = StashInterface(config.STASH_CONFIG)
except Exception:
    stash = None
...
def _init_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS monitored_messages (
            ...
            discovered_at REAL,
            UNIQUE(chat_id, msg_id, tag)
        )
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_mon_tag ON monitored_messages(tag, is_queued, is_transferred)")
    
    for stmt in ("ALTER TABLE monitored_messages ADD COLUMN is_transferred INTEGER DEFAULT 0",):
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS tg_routing_destinations (
            ...
            topic_title TEXT,
            created_at REAL,
            PRIMARY KEY (tag, forum_id)
        )
    """)
    
    for stmt in ("ALTER TABLE tg_routing_destinations ADD COLUMN forum_id INTEGER DEFAULT 0",):
        try:
            conn.execute(stmt)
            conn.execute("DROP INDEX IF EXISTS sqlite_autoindex_tg_routing_destinations_1")
        except sqlite3.OperationalError: pass
...
# ============================================================
# STASH INTEGRATION GLOBALS & HELPERS
# ============================================================
PERFORMER_CACHE = {}

async def load_stash_performers():
    """Fetches Stash performers asynchronously and caches them."""
    global PERFORMER_CACHE
    if not stash:
        console.print("[yellow]⚠️ Stash interface unavailable. Skipping cache sync.[/yellow]")
        return 0
    
    console.print("[cyan]🍓 Syncing Stash performers reference database...[/cyan]")
    def _fetch():
        cache = {}
        try:
            ...
            for p in performers:
                ...
                if name:
                    cache[name.lower().strip()] = f"#{tag_name}"
                for alias in aliases:
                    if alias:
                        cache[alias.lower().strip()] = f"#{tag_name}"
        except Exception as e:
            console.print(f"[red]❌ Stash sync failed during fetch: {e}[/red]")
        return cache
        
    PERFORMER_CACHE = await asyncio.to_thread(_fetch)
    console.print(f"[bold green]✅ Stash cache populated with {len(PERFORMER_CACHE)} name bindings.[/bold green]")
    return len(set(PERFORMER_CACHE.values()))

def extract_actor_tags(text):
    """Hybrid tag extractor checking both explicit #tags and the Stash cache."""
    found_tags = set()
    if not text:
...
# Replace the older extract_all_tags globally to instantly benefit from Stash logic
def extract_all_tags(text):
    return extract_actor_tags(text)
...
async def tg_get_routing_config(tag, forum_id):
    return await db_execute(
        "SELECT topic_id, topic_title FROM tg_routing_destinations WHERE tag=? AND forum_id=?",
        (tag, forum_id), fetch="one"
    )

async def tg_set_routing_config(tag, forum_id, topic_id, topic_title):
    await db_execute(
        """INSERT INTO tg_routing_destinations (tag, forum_id, topic_id, topic_title, created_at)
           VALUES (?,?,?,?,?)
           ON CONFLICT(tag, forum_id) DO UPDATE SET topic_id=excluded.topic_id, topic_title=excluded.topic_title""",
        (tag, forum_id, topic_id, topic_title, time.time())
    )
...
async def tg_create_destination_topic(tag, target_forum_id=None):
    forum_id_str = target_forum_id or await get_control("master_forum_id")
    if not forum_id_str:
        raise Exception("Forum ID is not set. Unable to route topics.")
    
    try: chat_identifier = int(forum_id_str)
    except ValueError: chat_identifier = forum_id_str
    ...
    if not topic_id: raise Exception("Topic was created in MTProto, but ID could not be parsed.")

    await asyncio.sleep(TRANSFER_STAGGER_SECONDS)
    await tg_set_routing_config(tag, real_chat_id, topic_id, title)
    return real_chat_id, topic_id, title

async def tg_resolve_destination_topic(tag, target_forum_id=None):
    forum_id_str = target_forum_id or await get_control("master_forum_id")
    if not forum_id_str:
        raise Exception("Forum ID is not set.")

    try: real_chat_id = int(forum_id_str)
    except ValueError:
        chat = await user_app.get_chat(forum_id_str)
        real_chat_id = chat.id
        if not target_forum_id: await set_control("master_forum_id", str(real_chat_id))

    dest = await tg_get_routing_config(tag, real_chat_id)
    if dest and dest[0]:  
        return real_chat_id, dest[0], dest[1]
    
    return await tg_create_destination_topic(tag, target_forum_id=real_chat_id)
...
# ============================================================
async def extract_and_store_message(chat_id, msg_id, text, file_unique_id, is_dropzone=False):
    tags = extract_actor_tags(text) if is_dropzone else re.findall(r"#([A-Za-z0-9_]+)", text)
    if not tags: return
    
    for raw_tag in tags:
        tag = f"#{raw_tag.lower().replace('#', '')}"
        await db_execute(
            """INSERT INTO monitored_messages (chat_id, msg_id, tag, file_unique_id, caption, discovered_at)
               VALUES (?,?,?,?,?,?)
               ON CONFLICT(chat_id, msg_id, tag) DO UPDATE SET caption=excluded.caption""",
...
async def scan_chat_history(chat_identifier, resolved_chat_id):
    ...
    try:
        ...
        if cid_str.lstrip('-').isdigit(): chat_obj = await user_app.get_chat(int(cid_str))
        else: chat_obj = await user_app.get_chat(cid_str)
        ...
        is_dropzone = str(resolved_chat_id) == str(getattr(config, 'DROPZONE_GROUP_ID', ''))

        async for message in user_app.get_chat_history(resolved_chat_id):
            if message.video or message.document:
                txt = message.caption or message.text or ""
                filename = message.video.file_name if (message.video and getattr(message.video, 'file_name', None)) else ""
                combined_text = f"{txt} {filename}" if is_dropzone else txt
                f_id = message.video.file_unique_id if message.video else (message.document.file_unique_id if message.document else "")
                await extract_and_store_message(resolved_chat_id, message.id, combined_text, f_id, is_dropzone=is_dropzone)
...
async def live_monitor_handler(client, message):
    ...
    dropzone_id = getattr(config, 'DROPZONE_GROUP_ID', None)
    is_dropzone = dropzone_id and str(chat_id) == str(dropzone_id)

    if is_dropzone:
        filename = message.video.file_name if (message.video and getattr(message.video, 'file_name', None)) else ""
        await extract_and_store_message(chat_id, message.id, f"{txt} {filename}", f_id, is_dropzone=True)

    row = await db_execute("SELECT chat_identifier FROM monitored_chats WHERE resolved_id=?", (chat_id,), fetch="one")
    if row and not is_dropzone:
...
def get_part_count(file_size):
    if time.time() < reduced_parallelism_until: return 1
                ...
                with open(part_file, "wb") as f:
                    async for chunk in client.stream_media(file_id, limit=chunk_limit, offset=chunk_offset):
                        ...
                        if len(buffer) >= MEM_BUFFER_SIZE:
                            f.write(buffer)
                            buffer.clear()
                        if time.time() < reduced_parallelism_until: await asyncio.sleep(0.3)
                    if buffer:
                        f.write(buffer)
                        buffer.clear()
...
async def async_fast_download(client, message, file_path, progress_callback, job_id):
    ...
    if not media: raise Exception("No valid media found in message.")
    ...
    for i, (chunk_offset, chunk_limit) in enumerate(ranges):
        ...
        if includes_final_chunk: expected_bytes = (chunk_limit - 1) * CHUNK_SIZE + last_chunk_size
        else: expected_bytes = chunk_limit * CHUNK_SIZE

        tasks.append(
            asyncio.create_task(
                _download_segment(
                    client=client, file_id=file_id, chunk_offset=chunk_offset, chunk_limit=chunk_limit,
                    part_file=part_files[i], job_id=job_id, tracker=tracker, expected_bytes=expected_bytes
                )
            )
        )
...
class ProgressFileReader(io.IOBase):
    ...
    def read(self, size=-1):
        ...
        if self._callback: self._callback(self._read_bytes, self._total)
        return chunk
...
def _sync_vk_upload(upload_url, file_path, progress_callback, job_id):
    ...
    try:
        ...
        if resp.status_code != 200: raise Exception(f"VK upload {resp.status_code}: {resp.text[:300]}")
        return resp.json()
    finally:
        reader.close()
...
async def _ensure_h264_aac(file_path):
    ...
    if v_codec == "h264" and (a_codec == "aac" or not a_codec): return file_path
    ...
    process = await asyncio.create_subprocess_exec(*conv_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    await process.wait()
...
async def download_worker(worker_id):
    while True:
        ...
        try:
            ...
            def dl_progress(current, total): update_metrics(dl_key, rich_task, "📥 DL", current, total)
            ...
            file_path = await async_fast_download(
                client=user_app, message=msg, file_path=target_file_path, progress_callback=dl_progress, job_id=job_id
            )
            ...
            if str(e) == "ForceAbort": delete_file_on_exit = True
            else: await update_job_status(job_id, "waiting")
        finally:
        ...
        try:
            ...
            if not file_path or not os.path.exists(file_path):
                ...
                if job.get('tier', 1) == 1:
                    ...
                    else: await download_queue_t1.put(job)
                else: await download_queue_t2.put(job)
                continue
            ...
            if not album_id or album_id <= 0:
                raise Exception(f"Invalid VK Album ID ({album_id}) for job {job_id}")
...
def _engine_banner():
    ...
    if engine_state == ENGINE_PAUSE_REQUESTED: return "🟡 Pause Requested — draining active transfers..."
    return "⏸️ PAUSED"
...
async def render_dashboard():
    ...
    if ui_state == "MAIN":
        ...
        if time.time() < reduced_parallelism_until: breaker_line += f"🚦 **Telegram flood-limit throttle active** — single-part DLs for ~{int(reduced_parallelism_until - time.time())}s\n"
        ...
        buttons.append([InlineKeyboardButton(f"📥 View DLs ({len(active_dls)})", callback_data="ui_DL_VIEW"), InlineKeyboardButton(f"📤 View UPs ({len(active_ups)})", callback_data="ui_UP_VIEW")])
        buttons.append([InlineKeyboardButton("🍓 Stash Engine", callback_data="ui_STASH_MAIN")])
    ...
    elif ui_state == "STASH_MAIN":
        dropzone_id = getattr(config, 'DROPZONE_GROUP_ID', None)
        if not dropzone_id:
            text = "❌ **Error:** `DROPZONE_GROUP_ID` is not configured in `config.py`."
            buttons.append([InlineKeyboardButton("🔙 Back to Main", callback_data="ui_MAIN")])
        else:
            tag_counts = await db_execute(
                "SELECT tag, COUNT(*) FROM monitored_messages WHERE chat_id=? AND is_transferred=0 GROUP BY tag ORDER BY COUNT(*) DESC LIMIT 15",
                (dropzone_id,), fetch="all"
            )
            total_pending = sum(count for _, count in (tag_counts or []))
            
            text = (f"🍓 **STASH TRIAGE ENGINE**\n"
                    f"━━━━━━━━━━━━━━━━━━\n"
                    f"Discovered Performers: {len(PERFORMER_CACHE)} | Pending Transfer: {total_pending}\n\n"
                    f"🏷️ **Pending Actor Breakdown:**\n")
            
            if not tag_counts:
                text += "✅ All caught up! No pending videos to transfer to Library.\n"
            else:
                for tag, count in tag_counts:
                    text += f"• `{tag}` ({count} pending)\n"
                    buttons.append([InlineKeyboardButton(f"{tag} ({count} pending)", callback_data=f"stash_review_{tag}")])
            
            text += "━━━━━━━━━━━━━━━━━━\n"
            buttons.append([InlineKeyboardButton("🚀 Run Batch Scan (Dropzone)", callback_data="stash_batch_scan")])
            buttons.append([InlineKeyboardButton("🔄 Resync Performers", callback_data="stash_resync")])
            buttons.append([InlineKeyboardButton("🔙 Back to Main", callback_data="ui_MAIN")])

    elif ui_state.startswith("STASH_REVIEW_"):
        tag = ui_state.replace("STASH_REVIEW_", "")
        dropzone_id = getattr(config, 'DROPZONE_GROUP_ID', None)
        
        count_row = await db_execute("SELECT COUNT(*) FROM monitored_messages WHERE chat_id=? AND tag=? AND is_transferred=0", (dropzone_id, tag), fetch="one")
        count = count_row[0] if count_row else 0
        
        if count == 0:
            text = f"✅ All videos for **{tag}** have already been transferred."
            buttons.append([InlineKeyboardButton("🔙 Back to Stash Menu", callback_data="ui_STASH_MAIN")])
        else:
            text = (f"🎬 **Actor:** {tag}\n"
                    f"📦 **Pending Videos:** {count}\n\n"
                    f"Do you want to copy these videos to the `{tag}` topic in your Library?")
            buttons.append([InlineKeyboardButton(f"🚀 Copy {count} Videos to Library", callback_data=f"stash_transfer_{tag}")])
            buttons.append([InlineKeyboardButton("🔙 Back to Stash Menu", callback_data="ui_STASH_MAIN")])
        ...
        total_tags_row = await db_execute("SELECT COUNT(DISTINCT tag) FROM monitored_messages WHERE is_queued=0 AND chat_id != ?", (getattr(config, 'DROPZONE_GROUP_ID', 0),), fetch="one")
        total_tags = total_tags_row[0] if total_tags_row else 0
        total_vids_row = await db_execute("SELECT COUNT(*) FROM monitored_messages WHERE is_queued=0 AND chat_id != ?", (getattr(config, 'DROPZONE_GROUP_ID', 0),), fetch="one")
        ...
        tags_data = await db_execute(
            "SELECT tag, COUNT(*) as cnt FROM monitored_messages WHERE is_queued=0 AND chat_id != ? GROUP BY tag ORDER BY cnt DESC LIMIT ? OFFSET ?",
            (getattr(config, 'DROPZONE_GROUP_ID', 0), PAGE_SIZE, monitor_page * PAGE_SIZE), fetch="all"
        )
    ...
    try:
        await bot_app.edit_message_text(
            chat_id=int(chat_id), message_id=int(msg_id), text=text, parse_mode=ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup(buttons)
        )
    except Exception: pass
...
async def dashboard_updater():
    ...
    while True:
        ...
        if engine_state == ENGINE_PAUSE_REQUESTED:
            no_active = not any(j['action'] == "📥 DL" for j in active_jobs.values()) and not any(j['action'] == "📤 UP" for j in active_jobs.values())
...
async def refresh_cmd(client, message):
    ...
    playlist_queues.clear(); playlist_order.clear(); cancelled_jobs.clear(); vk_video_title_cache.clear(); vk_album_name_cache.clear()
    await db_execute("DELETE FROM jobs WHERE status != 'done'")
    await db_execute("UPDATE monitored_messages SET is_queued=0")

    await status_msg.edit_text("🔄 **Initiating System & VK Database Refresh...**\n`[3/5]` Querying live VK catalog...", parse_mode=ParseMode.MARKDOWN)
    synced_cnt, dupes_cnt, broken_cnt = await sync_vk_to_local_db()

    await status_msg.edit_text("🔄 **Initiating System & VK Database Refresh...**\n`[4/5]` Resynchronizing monitored tags...", parse_mode=ParseMode.MARKDOWN)
    await asyncio.sleep(1)
...
async def monitor_cmd(client, message):
    ...
    await message.reply_text("👁️ **MONITORING CONFIGURATION**\nSend the Group IDs or usernames you want to monitor for VK.", parse_mode=ParseMode.MARKDOWN)
...
@bot_app.on_message(filters.command("stash"))
async def stash_cmd(client, message):
    global ui_state
    ui_state = "STASH_MAIN"
    await render_dashboard()
...
async def set_master_forum_cmd(client, message):
    if message.chat.type in (enums.ChatType.GROUP, enums.ChatType.SUPERGROUP):
        ...
        if not getattr(full_chat, "is_forum", False):
            return await message.reply_text("⚠️ **Topics are not enabled here.**\nPlease go to Group Settings -> Enable 'Topics'.", parse_mode=ParseMode.MARKDOWN)
    else:
...
async def _resolve_target_chat(message, command_name):
    ...
    try:
        if raw_target.lstrip("-").isdigit(): chat_obj = await user_app.get_chat(int(raw_target))
        else: chat_obj = await user_app.get_chat(raw_target)
...
@bot_app.on_message(filters.private & filters.text & ~filters.command(["start", "help", "refresh", "monitor", "monitoralways", "monitorselected", "transfer", "copy", "autotransfer", "reset", "setmasterforum", "stash"]))
async def handle_user_input(client, message):
    ...
    if state.get('awaiting_sel_groups'):
        ...
        for target in raw_inputs:
            try:
                if target.startswith("-100") or target.startswith("-") or target.isdigit(): chat_obj = await user_app.get_chat(int(target))
                else: chat_obj = await user_app.get_chat(target)
                resolved_id = chat_obj.id
    ...
    kbd = InlineKeyboardMarkup([[InlineKeyboardButton(f"🚀 Queue {len(new_msgs)} videos for VK", callback_data="queue_transfer")], [InlineKeyboardButton(f"📋 Copy to Master Forum", callback_data="direct_copy_forum")], [InlineKeyboardButton(f"🚀 Transfer to Master Forum (Delete Orig)", callback_data="direct_transfer_forum")]])
...
async def handle_buttons(client, callback):
    ...
    if data.startswith("ui_"):
    ...
    elif data == "stash_resync":
        await callback.answer("Resyncing with Stash API...", show_alert=False)
        await load_stash_performers()
        await render_dashboard()
        return
        
    elif data == "stash_batch_scan":
        dropzone_id = getattr(config, 'DROPZONE_GROUP_ID', None)
        if not dropzone_id: return await callback.answer("Dropzone not configured.", show_alert=True)
        await callback.answer("Triggering background scan of Dropzone...", show_alert=False)
        asyncio.create_task(scan_chat_history("DROPZONE", dropzone_id))
        return

    elif data.startswith("stash_transfer_"):
        tag = data.replace("stash_transfer_", "")
        dropzone_id = getattr(config, 'DROPZONE_GROUP_ID', None)
        library_id = getattr(config, 'LIBRARY_GROUP_ID', None)
        if not dropzone_id or not library_id:
            return await callback.answer("Missing DROPZONE_GROUP_ID or LIBRARY_GROUP_ID in config.", show_alert=True)

        await callback.answer(f"Starting transfer for {tag} to Library...", show_alert=False)
        await callback.message.edit_text(f"⏳ **Starting transfer for {tag} to Library...**\nPlease wait.")

        try:
            _, topic_id, _ = await tg_resolve_destination_topic(tag, target_forum_id=library_id)
            topic_cache = await build_topic_dedupe_cache(library_id, topic_id)

            rows = await db_execute("SELECT msg_id FROM monitored_messages WHERE chat_id=? AND tag=? AND is_transferred=0", (dropzone_id, tag), fetch="all")
            pending_messages = [r[0] for r in rows] if rows else []
            
            success_count = 0
            for msg_id in pending_messages:
                try:
                    m = await user_app.get_messages(dropzone_id, msg_id)
                    if m and (m.video or m.document):
                        if await tg_execute_message_copy(dropzone_id, m, library_id, topic_id, tag, delete_original=False, topic_cache=topic_cache):
                            success_count += 1
                        await db_execute("UPDATE monitored_messages SET is_transferred=1 WHERE chat_id=? AND msg_id=? AND tag=?", (dropzone_id, msg_id, tag))
                except Exception as e:
                    console.print(f"[red]⚠️ Stash transfer failed for msg {msg_id}: {e}[/red]")
            
            await callback.message.edit_text(f"✅ **Transfer Complete!**\nSuccessfully copied {success_count} videos into the `{tag}` folder in your Library.")
            await asyncio.sleep(2)
        except Exception as e:
            await callback.message.edit_text(f"❌ **Error during Library transfer:** {e}")
            await asyncio.sleep(3)
        ...
        if not target_chat_id: return await callback.answer("No valid chat ID found in messages.", show_alert=True)
        ...
        if mode == "TAGS":
            ...
            await callback.message.edit_text("🏷️ **Tag Filter Mode**\nPlease type the tags you want to process, separated by commas.", parse_mode=ParseMode.MARKDOWN)
            return
        if state['cmd_type'] == "autotransfer":
            kbd = InlineKeyboardMarkup([[InlineKeyboardButton("🗑️ Delete originals", callback_data="atr_setup_del")], [InlineKeyboardButton("📋 Keep originals", callback_data="atr_setup_keep")]])
        ...
        c_id, title, mode = state['target_chat_id'], state['target_title'], state['mode']
        ...
        await db_execute("UPDATE autotransfer_monitors SET status='ACTIVE' WHERE chat_id=?", (int(data.replace("atr_proceed_", "")),))
        await callback.answer("▶️ Resumed Auto-Transfer.")
        await render_dashboard()

    elif data.startswith("atr_pause_"):
        await db_execute("UPDATE autotransfer_monitors SET status='PAUSED' WHERE chat_id=?", (int(data.replace("atr_pause_", "")),))
        await callback.answer("⏸️ Paused Auto-Transfer.")
        await render_dashboard()

    elif data.startswith("atr_stop_"):
        await db_execute("DELETE FROM autotransfer_monitors WHERE chat_id=?", (int(data.replace("atr_stop_", "")),))
        ...
        while not upload_queue.empty(): upload_queue.get_nowait(); upload_queue.task_done()
        playlist_queues.clear(); playlist_order.clear(); cancelled_jobs.clear(); vk_video_title_cache.clear(); vk_album_name_cache.clear()
        active_jobs.clear(); active_bulk_tasks.clear(); user_states.clear(); vk_reupload_attempts.clear()
        ...
        await callback.message.reply_text("📌 Send the Group ID(s) or @username(s) to monitor selectively:")
        return await callback.answer()
        ...
        await db_execute("UPDATE selected_monitors SET status='ACTIVE' WHERE chat_id=?", (int(data.replace("sel_proceed_", "")),))
        await callback.answer("▶️ Resumed Selective Auto-Sync.")
        await render_dashboard()

    elif data.startswith("sel_pause_"):
        await db_execute("UPDATE selected_monitors SET status='PAUSED' WHERE chat_id=?", (int(data.replace("sel_pause_", "")),))
        await callback.answer("⏸️ Paused Selective Auto-Sync.")
        await render_dashboard()

    elif data.startswith("sel_stop_"):
        await db_execute("DELETE FROM selected_monitors WHERE chat_id=?", (int(data.replace("sel_stop_", "")),))
        await callback.answer("🛑 Selective Sync Stopped.")
        await render_dashboard()

    elif data.startswith("always_proceed_"):
        await db_execute("UPDATE always_monitors SET status='ACTIVE' WHERE chat_id=?", (int(data.replace("always_proceed_", "")),))
        await callback.answer("▶️ Resumed Auto-Sync.")
        await render_dashboard()

    elif data.startswith("always_pause_"):
        await db_execute("UPDATE always_monitors SET status='PAUSED' WHERE chat_id=?", (int(data.replace("always_pause_", "")),))
        await callback.answer("⏸️ Paused Auto-Sync.")
        await render_dashboard()

    elif data.startswith("always_stop_"):
        await db_execute("DELETE FROM always_monitors WHERE chat_id=?", (int(data.replace("always_stop_", "")),))
        ...
        return await callback.message.reply_text("🔍 Please type the hashtag you want to search for:")

    elif data.startswith("mon_inspect_"):
        ui_state = f"MON_INSPECT_{data.replace('mon_inspect_', '')}"
        await render_dashboard()
        return await callback.answer()
        ...
        rows = await db_execute("SELECT chat_id, msg_id, caption FROM monitored_messages WHERE tag=? AND is_queued=0 AND chat_id != ?", (tag, getattr(config, 'DROPZONE_GROUP_ID', 0)), fetch="all")
        if not rows:
        ...
        for c_id, m_id, caption, idx in valid_jobs_data:
            job_id, is_pilot = f"{c_id}_{m_id}", not pilot_added
            if is_pilot: pilot_added = True
            job = {
                'job_id': job_id, 'playlist_id': playlist_id, 'chat_id': chat_id, 'msg_chat_id': c_id, 'msg_id': m_id, 'album_id': album_id,
                'album_name': album_name, 'query': tag, 'idx': idx, 'is_pilot': is_pilot, 'status': 'waiting', 'file_path': None, 'caption': caption, 'tier': 1
            }
            await save_job(job)
        ...
        await kill_playlist(data[len("plkill_"):])
        ...
        while not download_queue_t1.empty():
            job = download_queue_t1.get_nowait(); download_queue_t1.task_done(); cancelled_jobs.add(job['job_id']); await delete_job_row(job['job_id']); cleared += 1
        while not download_queue_t2.empty():
            job = download_queue_t2.get_nowait(); download_queue_t2.task_done(); cancelled_jobs.add(job['job_id']); await delete_job_row(job['job_id']); cleared += 1
        for pid in list(playlist_queues.keys()):
            q = playlist_queues.pop(pid)
            for job in q: cancelled_jobs.add(job['job_id']); await delete_job_row(job['job_id']); cleared += 1
        ...
        album_id = await get_or_create_vk_album(album_name)
        if not album_id: return await callback.message.edit_text("❌ Failed to resolve VK album.")
        await refresh_vk_cache(album_id)
        ...
        if not non_dupe_msgs:
            user_states.pop(chat_id, None); await callback.message.delete(); await render_dashboard()
            return
        ...
        for msg in non_dupe_msgs:
            ...
            job = {
                'job_id': job_id, 'playlist_id': playlist_id, 'chat_id': chat_id, 'msg_chat_id': msg.chat.id, 'msg_id': msg.id, 'album_id': album_id,
                'album_name': album_name, 'query': state['query'], 'idx': idx, 'is_pilot': is_pilot, 'status': 'waiting', 'file_path': None, 'caption': getattr(msg, '_custom_caption', ""), 'tier': 1
            }
            await save_job(job)
            cancelled_jobs.discard(job_id)
            if is_pilot:
...
async def main():
    ...
    await bot_app.set_bot_commands([
        ...
        BotCommand("stash", "🍓 Open Stash Triage Engine"),
    ...
    if dashboard_chat_id:
        try:
            dash_msg = await bot_app.send_message(
                chat_id=int(dashboard_chat_id), text="⚙️ **System Online / Reboot Detected**\nBooting Master Dashboard...", parse_mode=ParseMode.MARKDOWN
            )
                    ...
                    if tag_clean:
                        ...
                        if album_id:
                            ...
                            if not await is_msg_in_db(c_id, msg.id):
                                job = {
                                    'job_id': job_id, 'playlist_id': None, 'chat_id': dashboard_chat_id or c_id, 'msg_chat_id': c_id, 'msg_id': msg.id, 'album_id': album_id,
                                    'album_name': tag_clean, 'query': f"#{tag_clean}", 'idx': 1, 'is_pilot': False, 'status': 'queued', 'file_path': None, 'caption': txt, 'tier': 2
                                }
                                await save_job(job)
                                if st == "ACTIVE": await download_queue_t2.put(job)
                                new_vids_cnt += 1
            if dashboard_chat_id:
                ...
                if msg.video or msg.document:
                    ...
                    for m in matched_msgs:
                        ...
                        if not await is_msg_in_db(c_id, m.id):
                            job = {
                                'job_id': job_id, 'playlist_id': None, 'chat_id': dashboard_chat_id or c_id, 'msg_chat_id': c_id, 'msg_id': m.id, 'album_id': album_id,
                                'album_name': tag_clean, 'query': f"#{tag_clean}", 'idx': idx, 'is_pilot': False, 'status': 'queued', 'file_path': None, 'caption': getattr(m, '_custom_caption', ""), 'tier': 2
                            }
                            await save_job(job)
                            if st == "ACTIVE": await download_queue_t2.put(job)
                            new_vids_cnt += 1
    ...
    if rows:
        for row in rows:
            job = {
                'job_id': row[0], 'playlist_id': row[1], 'chat_id': row[2], 'msg_chat_id': row[3], 'msg_id': row[4], 'album_id': row[5], 'album_name': row[6], 'query': row[7],
                'idx': row[8], 'is_pilot': bool(row[9]), 'file_path': row[11], 'caption': row[12], 'tier': row[13] if len(row) > 13 else 1
            }
            ...
            if status in ("downloaded", "uploading") and file_path and os.path.exists(file_path):
                if await _validate_video_file(file_path):
                    ...
                    if job['is_pilot']:
                        await update_job_status(job['job_id'], "queued"); await target_q.put(job)
