...
async def render_dashboard():
    ...
    if ui_state == "MAIN":
        ...
        for c_id, c_title, st in (groups or []):
            ...
            if st == "ACTIVE":
                buttons.append([
                    InlineKeyboardButton(f"⏸️ Pause {c_title[:12]}", callback_data=f"sel_pause_{c_id}"),
                    InlineKeyboardButton(f"🛑 Stop", callback_data=f"sel_stop_{c_id}")
                ])
            else:
                buttons.append([
                    InlineKeyboardButton(f"▶️ Proceed {c_title[:12]}", callback_data=f"sel_proceed_{c_id}"),
                    InlineKeyboardButton(f"🛑 Stop", callback_data=f"sel_stop_{c_id}")
                ])
...
async def handle_user_input(client, message):
    ...
    if state.get('awaiting_sel_groups'):
        ...
        status_msg = await message.reply_text("⚙️ Adding selective monitoring groups...")
        ...
        for target in raw_inputs:
            try:
                ...
                console.print(f"[bold red]Failed to add selective group {target}: {e}[/bold red]")

        await status_msg.edit_text(f"✅ Added **{added_cnt}** selective group(s)!")
        global ui_state
        ui_state = "SELECTED_VIEW"
        await render_dashboard()
        return
