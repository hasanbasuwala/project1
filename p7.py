...
def _init_db():
    ...
    # ALWAYS MONITORS TABLE (FOR /MonitorAlways FEATURE)
    conn.execute("""
...
# ============================================================
# 2-TIER QUEUE SYSTEM (Tier 1 = High Priority / Manual, Tier 2 = Continuous / Background)
...
async def get_or_create_vk_album(album_name):
    ...
    try:
        ...
        for alb in existing.get('items', []):
            title = alb.get('title', '')
            vk_album_name_cache[title.strip().lower()] = alb['id']
            if title.strip().lower() == norm_name:
                return alb['id']

        new_alb = await asyncio.to_thread(vk.video.addAlbum, title=album_name)
        alb_id = new_alb if isinstance(new_alb, int) else new_alb['album_id']
        vk_album_name_cache[norm_name] = alb_id
        return alb_id
    except Exception as e:
        console.print(f"[bold red]❌ Failed to resolve/create VK album '{album_name}': {e}[/bold red]")
        return None
...
async def live_monitor_handler(client, message):
    ...
    # 1. Standard Monitor Tag Storage
    ...
    # 2. Continuous Always-Monitor Processing
    ...
    if always_row and always_row[0] == "ACTIVE":
        ...
        if await is_msg_in_db(chat_id, message.id):
            return
        ...
        console.print(f"[bold green]📡 Continuous Auto-Sync queued message {message.id} from chat {chat_id}[/bold green]")
...
async def scheduler_loop():
    while True:
        ...
        # Check Tier 1 (Manual Playlists) first
...
async def download_worker(worker_id):
    while True:
        ...
        # 2-Tier Drain Logic: Pull from Tier 1 (Manual) first, fall back to Tier 2 (Continuous)
        try:
...
async def start_cmd(client, message):
    ...
    await message.reply_text("👋 Dashboard Ready! Use /monitor or /MonitorAlways to setup group scanning.")
...
@bot_app.on_message(filters.command("MonitorAlways") | filters.command("monitoralways"))
async def monitor_always_cmd(client, message):
    ...
    await message.reply_text(
        ...
        "Please send the Group ID or public @username to continuously mirror to VK.\n"
        "*(Every video with a `#tag` will automatically create/route to its VK Album).*",
        parse_mode=ParseMode.MARKDOWN
    )

@bot_app.on_message(filters.private & filters.text & ~filters.command(["start", "help", "monitor", "MonitorAlways", "monitoralways"]))
async def handle_user_input(client, message):
    ...
    if state.get('awaiting_always_input'):
        ...
        status_msg = await message.reply_text("⚙️ Connecting to group and setting up continuous auto-sync...")
        ...
        try:
            ...
            # Discover historical videos
            found_cnt = 0
            async for msg in user_app.get_chat_history(resolved_id, limit=500):
            ...
            kbd = InlineKeyboardMarkup([
                [InlineKeyboardButton("⏸️ Pause Sync", callback_data=f"always_pause_{resolved_id}"),
                 InlineKeyboardButton("🛑 Stop Sync", callback_data=f"always_stop_{resolved_id}")]
            ])
            await status_msg.edit_text(f"✅ **Continuous Auto-Sync Active!**\nGroup: **{title}**\nDiscovered `{found_cnt}` historical videos.", reply_markup=kbd)
        except Exception as e:
...
async def handle_buttons(client, callback):
    ...
    if data.startswith("ui_"):
        ...
        await callback.answer("▶️ Resumed Continuous Auto-Sync.")
        await render_dashboard()
        ...
        await callback.answer("⏸️ Paused Continuous Auto-Sync.")
        await render_dashboard()
        ...
        await callback.answer("🛑 Continuous Auto-Sync Stopped.")
        await render_dashboard()
...
async def main():
    ...
    # Reboot Scan for Always-Monitors (/MonitorAlways)
    ...
    with Live(progress_ui, console=console, refresh_per_second=4):
        console.print("[bold green]🚀 Master Engine Online. Send /start in bot![/bold green]")
