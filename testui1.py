...
async def generate_xray_dump(url: str, user_id: str) -> str:
    """Scrapes the raw DOM of the main page and all iframes, saves to a file."""
    dump_path = f"dom_dump_{user_id}.txt"
    USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
    ...
    async with async_playwright() as p:
        ...
        try:
            await page.goto(url, wait_until="domcontentloaded", timeout=45000)
            await page.wait_for_timeout(8000) # Give Cloudflare Turnstile time to clear

            with open(dump_path, "w", encoding="utf-8") as f:
                f.write(f"=== MAIN PAGE: {page.url} ===\n\n")
                f.write(await page.content() + "\n\n")

                for i, frame in enumerate(page.frames):
                    try:
                        f.write(f"=== IFRAME {i}: {frame.url} ===\n\n")
                        f.write(await frame.content() + "\n\n")
                    except Exception as e:
                        f.write(f"Could not extract frame: {e}\n\n")
                        
        except Exception as e:
            with open(dump_path, "w", encoding="utf-8") as f:
                f.write(f"CRITICAL ERROR DURING X-RAY: {e}")
...
def setup_router(app: Client, db: JobScheduler, pipeline: PipelineManager):
    ...
    @app.on_message(filters.command("xray"))
async def xray_command(client, message):
    # Ensure they provided a URL
    if len(message.command) < 2:
        await message.reply_text("⚠️ **Usage:** `/xray <url>`")
        return
    ...
    status_msg = await message.reply_text(f"🔬 **X-Ray Scanner Engaged**\nPenetrating Cloudflare and dumping DOM for: {target_url}\n\n*Please wait ~15 seconds...*")
    
    try:
        # Run the Playwright payload
        dump_file_path = await generate_xray_dump(target_url, user_id)
        
        # Upload the text file to Telegram
        await message.reply_document(
            document=dump_file_path, 
            caption="✅ **X-Ray Complete.**\nHere is the raw HTML dump of the main page and all embedded iframes.\n\nOpen this file and search for `button`, `submit`, or `form`."
        )
        
    except Exception as e:
        await message.reply_text(f"❌ **X-Ray Failed:** `{e}`")
        
    finally:
        # Clean up the loading message and delete the text file from your server
