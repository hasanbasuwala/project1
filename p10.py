...
async def sync_vk_to_local_db():
    """Fetches all albums and videos from VK on startup, seeds SQLite with 'done' records, 
    and automatically purges duplicate [TG_ID] videos found in VK."""
    console.print("[bold cyan]🔄 Syncing state from VK and cleaning duplicate uploads...[/bold cyan]")
    synced_count = 0
    ...
    try:
        ...
        # Include None to also check unassigned/orphan videos in VK
            ...
            while True:
                ...
                for v in video_list:
                    ...
                    if match:
                        ...
                        # If we've already indexed this TG message ID, this video is a duplicate!
                        if msg_id in seen_msg_ids:
                            try:
                                ...
                                console.print(f"[bold red]❌ Failed to delete duplicate VK video {video_id}: {e}[/bold red]")
                            continue
        ...
        console.print(f"[bold green]✅ VK Sync Complete: {synced_count} unique videos indexed, {deleted_duplicates_count} duplicates purged from VK.[/bold green]")
    except Exception as e:
...
def extract_all_tags(text):
    if not text:
        return []
    # Ensures case-insensitivity at the root of extraction
...
async def live_monitor_handler(client, message):
    ...
    # 2. Continuous Always-Monitor Processing (/MonitorAlways)
    ...
    # 3. Selective Monitor Processing (/MonitorSelected)
...
async def download_worker(worker_id):
    while True:
        ...
        try:
            ...
            # Network-resilient message fetch
            msg = None
...
@bot_app.on_message(filters.private & filters.text & ~filters.command(["start", "help", "monitor", "monitoralways", "monitorselected"]))
async def handle_user_input(client, message):
    chat_id = message.chat.id
    state = user_states.get(chat_id, {})

    # Handling Selective Group Input
    if state.get('awaiting_sel_groups'):
        ...
        for target in raw_inputs:
            try:
                ...
                # Run immediate historical scan
                tot, in_vk, valid_jobs = await run_selective_history_scan(resolved_id)

                if valid_jobs:
                    # Stash jobs temporarily in user state for confirmation button
    ...
    # Lowercasing implemented here to ensure manual search fully matches exact casing logic
...
async def handle_buttons(client, callback):
    ...
    # Manual Scan Trigger for Selective Groups
...
async def main():
    ...
    # 3. Resume Background Historical Scans
    ...
    # 4. Auto-Dashboard Initialization & Auto-Pin on Startup
    ...
    # 5. Reboot Scan for Always-Monitors (/MonitorAlways)
    ...
    # 6. Reboot Scan for Selective Monitors (/MonitorSelected)
