...
# CH 10 — /scan #tag & /deepscan #tag SESSION STATE + RENDERERS
# ═══════════════════════════════════════════════════════════════════════
def render_deepscan_ui(chat_id: int):
    ...
    text = (f"{TXT.DSCAN_RESULTS_TITLE}\n🏷 Tag: `#{tag}`\n"
            f"{TXT.DSCAN_ROW_COMMUNITIES} `{len(communities)}`  |  {TXT.DSCAN_ROW_TOTAL_VIDEOS} `{total_videos}`\n"
            f"{TXT.DIVIDER}")
...
def setup_router(app: Client, db: JobScheduler, dl_q: asyncio.Queue, enc_q: asyncio.Queue, up_q: asyncio.Queue, dl_pool: "WorkerPool", enc_pool: "WorkerPool", up_pool: "WorkerPool"):
    ...
    async def refresh_dashboard_if_open():
        ...
        if _dash_msg_id and _dash_chat_id:
            dash_text, dkb = await render_dashboard(db, _dash_tab, _expanded_pl, _expanded_bucket, _expanded_jid)
            await safe_edit(app, _dash_chat_id, _dash_msg_id, dash_text, dkb)
    ...
    async def run_deepscan(chat_id: int, msg_id: int, tag: str):
        try:
            ...
            pattern = _hashtag_pattern(tag)
            communities, total_videos, last_edit = [], 0, 0.0
            ...
            for i, g in enumerate(groups):
                ...
                try:
                    ...
                    vids = []
                if vids:
                    communities.append({'id': comm_id, 'name': g.get('name', f"Community {g['id']}"), 'videos': vids})
                ...
                if now - last_edit >= PROGRESS_EDIT_MIN_INTERVAL_SEC or i == total_groups - 1:
                    ...
                    await safe_edit(app, chat_id, msg_id,
                        f"{TXT.DSCAN_PROGRESS_TITLE}\n{TXT.DIVIDER}\n"
                        f"{TXT.DSCAN_ROW_TAG} `#{tag}`\n`[{make_bar(pct, 16)}] {pct:.0f}%`\n"
                        f"{TXT.DSCAN_ROW_SCANNED} `{i+1}/{total_groups}` {TXT.DSCAN_ROW_HITS_UNIT}\n"
                        f"{TXT.DSCAN_ROW_HITS} `{len(communities)}` {TXT.DSCAN_ROW_HITS_UNIT} · `{total_videos}` {TXT.DSCAN_ROW_HITS_VIDEO_UNIT}")
                    last_edit = now

            if not communities:
                return await safe_edit(app, chat_id, msg_id, TXT.DSCAN_NONE_FOUND.format(tag=tag, total=total_groups))

            _deepscan_sessions[chat_id] = {'tag': tag, 'communities': communities, 'expanded': set()}
            text, kb = render_deepscan_ui(chat_id)
