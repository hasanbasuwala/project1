"""
vk_bot.py - Dedicated VK-to-VK Playlist Deduplicator & Downloader
───────────────────────────────────────────────────────────────
FEATURES & ARCHITECTURE (v3 - VK-to-VK Edition):
  • VK-to-VK Pipeline: Downloads from VK, skips FFmpeg entirely, and uploads 
    directly back to your VK account using aiohttp.
  • Album & Deduplication: Target albums using `#AlbumName`. The script resolves 
    the album, scans descriptions for [VK_DB_ID: x_y], and skips duplicates.
  • Zero-Cost Encode: Transcoding is bypassed. Raw files are passed directly 
    to VK's ingestion servers for native processing.
  • Graceful Cancel: Stops new downloads but safely finishes uploading any 
    videos already downloaded to disk.
  • Detailed Reboot Report: Posts a full breakdown of the queue state to Telegram 
    if the bot recovers from a crash or system reboot.
───────────────────────────────────────────────────────────────
"""
...
def clean_title(title_str: str) -> str:
    """Strips the internal ID delimiter for UI display."""
...
def get_or_create_vk_album(vk, album_name: str) -> int:
    """Finds an existing album by name (case-insensitive) or creates it."""
    albums = vk.video.getAlbums(count=100).get('items', [])
    for album in albums:
        if album.get('title', '').strip().lower() == album_name.strip().lower():
            return album['id']
    new_album = vk.video.addAlbum(title=album_name)
    return new_album['album_id']


def get_existing_vk_db_ids(vk, album_id: int) -> set:
    """Scans a VK album and extracts all unique source IDs from descriptions."""
    existing_ids = set()
    offset = 0
    count = 100
    while True:
...
class JobScheduler:
    ...
    async def graceful_cancel_playlist(self, pl_id: str):
        async with self.lock:
            self.conn.execute('DELETE FROM playlist_items WHERE playlist_id = ? AND status = "pending"', (pl_id,))
            cancel_jobs = [dict(r) for r in self.conn.execute(
                'SELECT id FROM jobs WHERE playlist_id = ? AND stage IN ("queued", "downloading")', (pl_id,)
            ).fetchall()]
            ...
            if remaining == 0:
                ...
                self.conn.execute('UPDATE playlists SET status = "cancelling" WHERE id = ?', (pl_id,))
            self.conn.commit()
    ...
    while True:
        ...
        try:
            ...
            for pl in active_playlists:
                if pl['status'] in ('paused', 'cancelled', 'completed', 'cancelling'):
                    continue
            ...
            eligible_playlists = [
                ...
                if pl['status'] not in ('paused', 'cancelled', 'completed', 'cancelling')
            ]
...
class DownloaderEngine:
    ...
    async def execute(self, job: dict):
        ...
        opts = {
            ...
            "merge_output_format": "mkv", # Let it use mkv if needed, VK handles it
            "quiet": False,
            "noprogress": True,
            "no_warnings": True,
...
class EncoderEngine:
    """Zero-Cost Pass-Through Engine. Moves yt-dlp output to upload dir."""
    async def execute(self, job: dict, db: JobScheduler):
        jid = job['id']
        dl_dir, enc_dir = JOBS_DIR / f"JOB_{jid}" / "dl", JOBS_DIR / f"JOB_{jid}" / "enc"
        
        existing_encoded = [f for f in enc_dir.rglob("*") if f.is_file()]
        if existing_encoded and existing_encoded[0].stat().st_size > 0:
            db.log_trace(jid, "Payload already queued for upload — skipping pass-through.")
            return
        ...
        db.log_trace(jid, f"VK-to-VK Architecture: Bypassing ffmpeg transcode. Moving {src.name} directly to upload stage.")
        src.rename(dst)
...
class UploaderEngine:
    """Uploads directly back to VK API using aiohttp."""
    def __init__(self, db: JobScheduler, app: Client):
        self.db = db
        self.app = app
    ...
    async def execute(self, job: dict):
        ...
        if not stage.startswith("uploaded"):
            ...
            if not files:
                raise RuntimeError("Payload missing from upload queue.")
            enc_file = files[0]
            ...
            raw_title = job['title']
            if "|||" in raw_title:
                clean_title_str, unique_id = raw_title.split("|||", 1)
            else:
                clean_title_str, unique_id = raw_title, "UNKNOWN"

            db_signature = f"\n\n[VK_DB_ID: {unique_id}]"

            def get_upload_server():
                VK_TOKEN = getattr(config, "VK_TOKEN", None)
                if not VK_TOKEN: raise ValueError("Missing VK_TOKEN in config.")
                import vk_api
                vk_session = vk_api.VkApi(token=VK_TOKEN)
                vk = vk_session.get_api()
                
                params = {
                    "name": clean_title_str,
                    "description": f"Archived via Stealth Bot{db_signature}"
                }
                if target_album_id:
            ...
            self.db.log_trace(jid, "Requesting VK upload server...")
            upload_data = await asyncio.to_thread(get_upload_server)
            upload_url = upload_data['upload_url']

            self.db.log_trace(jid, "Streaming payload to VK...")
...
def render_worker_panel(dl_pool: "WorkerPool", enc_pool: "WorkerPool", up_pool: "WorkerPool") -> tuple[str, InlineKeyboardMarkup]:
    ...
    kb = [
        ...
        [
            ...
            InlineKeyboardButton(f"⚙️ PREP: {enc_pool.target}", callback_data="noop"),
            InlineKeyboardButton("+", callback_data="wk|enc|1"),
        ],
        [
...
async def render_dashboard(db: JobScheduler, tab: str = "playlists", exp_pl: str = None, exp_bucket: str = None, exp_jid: str = None) -> tuple[str, InlineKeyboardMarkup]:
    ...
    text = (
        ...
        f"`[🏁] LAST :` `{clean_title(_last_completed)[:12]}`\n`━━━━━━━━━━━━━━━━━━━━━━━━━━`"
    )
    ...
    if playlists:
        ...
        if any_active: kb.append([InlineKeyboardButton("⏸ PAUSE ALL PLAYLISTS", callback_data="pause_all")])
    ...
    if is_root_open:
        if not playlists: kb.append([InlineKeyboardButton("└ System Idle (Send Link #AlbumName)", callback_data="noop")])
        else:
            for pl in playlists:
                pl_id = pl['id']
                ...
                # If cancelling, show it clearly
                status_txt = " [CANCELLING]" if pl['status'] == "cancelling" else ""

                kb.append([InlineKeyboardButton(
                    f" {'[-]' if is_this_pl_exp else '[+]'} {pl_status_icon} Album: {pl['caption'][:10] or 'Default'}{status_txt} [{pl['downloaded']}/{pl['total']}]",
                ...
                if is_this_pl_exp:
                    ...
                    def build_bucket(bucket_id, label, icon, job_list):
                        ...
                        if is_b_open:
                            ...
                            for j in job_list:
                                ...
                                clean_t = clean_title(j['title'])
                                if is_j_open:
                                    speed, eta = "—", "—"
                                    p = [x.strip() for x in (j.get('stage') or "").split("|")]
                                    if len(p) >= 3: speed, eta = p[1], p[2]
                                    elif len(p) == 2: speed = p[1]
                                    pct = float(j.get('pct', 0.0) or 0.0)
                                    kb.append([InlineKeyboardButton(f"🪪 JOB: {jid}", callback_data="noop")])
                                    kb.append([InlineKeyboardButton(f"📁 {clean_t[:15]}...", callback_data="noop")])
                                    kb.append([InlineKeyboardButton(f"⚡ {speed}  |  ⏳ {eta}", callback_data="noop")])
                                    kb.append([InlineKeyboardButton(f"📊 [{make_bar(pct, 8)}] {pct:.1f}%", callback_data="noop")])
                                    kb.append([InlineKeyboardButton("📄 LOGS", callback_data=f"joblog|{jid}"), InlineKeyboardButton("❌ KILL", callback_data=f"kill_job|{jid}")])
                                    kb.append([InlineKeyboardButton("🔙 CLOSE CARD", callback_data=f"dash|playlists:{pl_id}:{bucket_id}")])
                                else:
                                    pct = float(j.get('pct', 0.0) or 0.0)
                                    kb.append([InlineKeyboardButton(f"      ├ ⚡ {clean_t[:10]}.. | {pct:.0f}%", callback_data=f"dash|playlists:{pl_id}:{bucket_id}:{jid}")])
                    ...
                    if pl['status'] == "cancelling":
                        kb.append([InlineKeyboardButton("⚠️ CANCELLING (Waiting on Uploads)", callback_data="noop")])
                    elif pl['status'] == "paused":
                        kb.append([InlineKeyboardButton("▶️ RESUME", callback_data=f"res|{pl['id']}"), InlineKeyboardButton("🧹 GRACEFUL CANCEL", callback_data=f"graceful_cancel|{pl['id']}")])
                    else:
                        kb.append([InlineKeyboardButton("⏸ PAUSE", callback_data=f"pause|{pl['id']}"), InlineKeyboardButton("🧹 GRACEFUL CANCEL", callback_data=f"graceful_cancel|{pl['id']}")])
...
def setup_router(app: Client, db: JobScheduler, dl_q: asyncio.Queue, enc_q: asyncio.Queue, up_q: asyncio.Queue,
    ...
    async def auto_catch_playlist(_, msg: Message):
        ...
        def extract_and_dedupe(playlist_url: str, album_name: str):
            VK_TOKEN = getattr(config, "VK_TOKEN", None)
            if not VK_TOKEN: raise ValueError("VK_TOKEN is missing in config.")

            import vk_api
            vk_session = vk_api.VkApi(token=VK_TOKEN)
            vk = vk_session.get_api()

            # 1. Resolve Album and Deduplication IDs
            ...
            if match:
                ...
                while True:
                    ...
                    for v in items:
                        ...
                        all_videos.append({
                            'url': f"https://vk.com/video{unique_id}", 
                            'title': v.get('title', 'VK Video'),
                            'unique_id': unique_id
                        })
                    offset += count
                ...
                if match_wall:
                    owner_id, post_id = match_wall.group(1), match_wall.group(2)
                    ...
                    with yt_dlp.YoutubeDL(opts) as ydl:
                        ...
                        for e in entries:
                            ...
                            unique_id = str(e.get('id', uuid.uuid4().hex[:10]))
                            if v_url: all_videos.append({'url': v_url, 'title': e.get('title', 'VK Video'), 'unique_id': unique_id})

            # 3. Filter Duplicates
            missing_videos = []
            for video in all_videos:
                if video['unique_id'] not in existing_ids:
    ...
    async def handle_callbacks(_, cb: CallbackQuery):
        ...
        if cb.data.startswith("wk|"):
            ...
            await cb.answer(
                "🧹 GRACEFUL CANCEL INITIATED\n\n"
                "• Pending downloads wiped.\n"
                "• In-progress downloads stopped.\n"
                "• Already-downloaded videos will finish uploading to VK, then close.",
                show_alert=True
            )
...
async def dashboard_refresher(app: Client, db: JobScheduler):
    ...
    while True:
        ...
        try:
            ...
            if set(current_hash.keys()) != set(k for k in last_state_hash.keys() if k != "playlists"):
                needs_update = True
...
async def send_reboot_crash_report(app: Client, db: JobScheduler):
    ...
    for pl in playlists:
        ...
        report_text = (
            ...
            f"  ├ ⏳ Remaining (Not Started): `{pending_items}`\n"
