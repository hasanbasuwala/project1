...
async def tg_resolve_destination_topic(tag):
    ...
    if dest and dest[0]:  
        return int(master_forum_id), dest[0], dest[1]
    
    return await tg_create_destination_topic(tag)

async def tg_execute_message_copy(src_chat_id, m, master_forum_id, topic_id, tag, delete_original):
    ...
    if m.media_group_id:
        ...
        for mid in ids:
            await tg_mark_message_copied(src_chat_id, mid, tag, topic_id)
            
        if delete_original:
            try: await user_app.delete_messages(src_chat_id, ids)
            except Exception: pass
    else:
        await tg_flood_safe(user_app.copy_message, master_forum_id, src_chat_id, m.id, reply_to_message_id=topic_id)
...
async def tg_scan_chat_for_tags(chat_id, limit=None):
    ...
    async for msg in user_app.get_chat_history(chat_id, **kwargs):
        ...
        for m in matched:
            ...
            if tag_clean:
                results.setdefault(f"#{tag_clean}", []).append(m)

    return results

async def tg_process_batch_transfer(chat_id, delete_originals, status_cb=None):
    tag_matches = await tg_scan_chat_for_tags(chat_id)
    if not tag_matches:
        return {}
    ...
    for tag, msgs in tag_matches.items():
        ...
        if status_cb:
            await status_cb(tag, topic_title, len(msgs))

        copied = 0
        for m in sorted(msgs, key=lambda x: x.id):
            try:
                if await tg_execute_message_copy(chat_id, m, master_forum_id, topic_id, tag, delete_originals):
                    copied += 1
            except Exception as e:
                console.print(f"[red]⚠️ Transfer failed for msg {m.id} ({tag}): {e}[/red]")

        summary[tag] = {"dest_title": topic_title, "dest_topic_id": topic_id, "copied": copied, "found": len(msgs)}
...
async def live_monitor_handler(client, message):
    ...
    # NEW: LIVE /autotransfer HANDLING (FORUM TOPICS)
    autotransfer_row = await db_execute(
        "SELECT status, delete_originals FROM autotransfer_monitors WHERE chat_id=?",
        (chat_id,), fetch="one"
    )
...
async def transfer_copy_cmd(client, message):
    ...
    status_msg = await message.reply_text(
        f"🔎 Scanning **{target_title}** for all registered tags ({'transfer' if delete_originals else 'copy'}) to Master Forum...",
        parse_mode=ParseMode.MARKDOWN
    )
    ...
    summary = await tg_process_batch_transfer(target_chat_id, delete_originals, status_cb=progress)

    if not summary:
        return await status_msg.edit_text("ℹ️ No registered tags matched anything in this chat.")
...
async def autotransfer_cmd(client, message):
    ...
    kbd = InlineKeyboardMarkup([[
        InlineKeyboardButton("🗑️ Delete originals", callback_data=f"atr_setup_del_{target_chat_id}"),
        InlineKeyboardButton("📋 Keep originals", callback_data=f"atr_setup_keep_{target_chat_id}")
    ]])
    await message.reply_text(
...
async def handle_user_input(client, message):
    ...
    kbd = InlineKeyboardMarkup([[InlineKeyboardButton(f"🚀 Queue {len(new_msgs)} videos for VK", callback_data="queue_transfer")]])
...
async def handle_buttons(client, callback):
    ...
    if data.startswith("ui_"):
        ...
        await db_execute(
            """INSERT INTO autotransfer_monitors (chat_id, chat_title, status, delete_originals, last_msg_id, added_at)
               VALUES (?,?,'ACTIVE',?,0,?)
               ON CONFLICT(chat_id) DO UPDATE SET status='ACTIVE', delete_originals=excluded.delete_originals""",
            (c_id, title, int(delete_originals), time.time())
        )
        ...
        async def _backfill():
            summary = await tg_process_batch_transfer(c_id, delete_originals)
