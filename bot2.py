...
class DownloaderEngine:
    ...
    async def _safe_fetch(self, context, page, url: str, headers: dict) -> tuple[bool, bytes]:
        """Attempts context fetch; falls back to native in-tab DOM fetch on 403."""
        try:
            resp = await context.request.get(url, headers=headers, timeout=25000)
            if resp.ok:
                return True, await resp.body()
            if resp.status != 403:
                return False, b""
        except Exception:
            pass # Socket hangup or timeout -> trigger DOM fallback

        # In-tab DOM Fetch Fallback
        try:
...
class PipelineManager:
    def __init__(self, app: Client, db: JobScheduler, vk_manager: VKPlaylistManager, browser_manager: PlaywrightBrowserManager):
        self.app, self.db = app, db
        self.dl_q, self.enc_q, self.up_q = asyncio.Queue(), asyncio.Queue(), asyncio.Queue()
        self.dl_engine = DownloaderEngine(db, app, browser_manager)
        self.enc_engine = EncoderEngine(db)
        self.up_engine = UploaderEngine(db, app, vk_manager)
