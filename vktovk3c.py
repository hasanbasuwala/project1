...
class TXT:
    ...
    ASCAN_STARTING           = "⏳ `Starting autoscan — scanning historical wall posts...`"
...
async def render_dashboard(db: JobScheduler, tab: str = "playlists", exp_pl: str = None, exp_bucket: str = None, exp_jid: str = None) -> tuple[str, InlineKeyboardMarkup]:
    ...
    if is_root_open:
        if not playlists: kb.append([InlineKeyboardButton(TXT.DASH_SYSTEM_IDLE_ROW, callback_data="noop")])
            ...
            for pl in playlists:
                ...
                kb.append([InlineKeyboardButton(f" {'[-]' if is_this_pl_exp else '[+]'} {pl_status_icon} {TXT.DASH_ALBUM_PREFIX} {pl['caption'][:10] or 'Default'}{status_txt} [{pl['downloaded']}/{pl['total']}]", callback_data=f"dash|playlists:{pl_id}" if not is_this_pl_exp else "dash|playlists")])
    ...
    # ---- AUTOSCAN dedicated section — always visible, not collapsible ----
    ...
    kb.append([InlineKeyboardButton("───────────────────", callback_data="noop")])
    kb.append([InlineKeyboardButton(f"{TXT.ASCAN_PANEL_HDR}  {TXT.ASCAN_PANEL_ON if ascan_enabled else TXT.ASCAN_PANEL_OFF}", callback_data="noop")])
...
async def deepscan_search_community(vk, comm_id: int, pattern: re.Pattern, max_posts: int = None) -> list[dict]:
    ...
    offset, count = 0, 100
    while True:
        res = await asyncio.to_thread(vk.wall.get, owner_id=comm_id, count=count, offset=offset)
        posts = res.get('items', [])
        if not posts: break
        for post in posts:
...
async def queue_tagged_videos(db: JobScheduler, chat_id: int, tag: str, videos: list[dict], source_label: str = None) -> tuple[int, int]:
    ...
    await db.create_playlist(pl_id, source_label or f"deepscan:#{tag}", str(target_album_id), len(missing), chat_id)
...
async def _resolve_autoscan_community_name(vk, comm_id: int, fallback: str) -> str:
    try:
        info = await asyncio.to_thread(vk.groups.getById, group_id=str(-comm_id))
...
async def _autoscan_full_scan_community(db: JobScheduler, vk, comm: dict, tag_rows: list[dict], progress_cb=None) -> int:
    ...
    while True:
        res = await asyncio.to_thread(vk.wall.get, owner_id=comm['resolved_id'], count=count, offset=offset)
        posts = res.get('items', [])
...
async def _autoscan_light_scan_community(db: JobScheduler, app: Client, vk, comm: dict, tag_rows: list[dict]) -> int:
    ...
    res = await asyncio.to_thread(vk.wall.get, owner_id=comm['resolved_id'], count=AUTOSCAN_MONITOR_MAX_POSTS, offset=0)
...
async def autoscan_cycle(db: JobScheduler, app: Client):
    ...
    for comm in communities:
        if not comm.get('resolved_id'):
            try:
                comm_id = await asyncio.to_thread(resolve_vk_community_id, vk, comm['url'])
...
def setup_router(app: Client, db: JobScheduler, dl_q: asyncio.Queue, enc_q: asyncio.Queue, up_q: asyncio.Queue, dl_pool: "WorkerPool", enc_pool: "WorkerPool", up_pool: "WorkerPool"):
    ...
    async def run_autoscan_historical(chat_id: int, msg_id: int, tags: list[str], comm_urls: list[str]):
        ...
        for url in comm_urls:
            try:
                comm_id = await asyncio.to_thread(resolve_vk_community_id, vk, url)
        ...
        for comm in resolved_comms:
            async def progress_cb(offset, total, queued_so_far, _comm=comm):
                pct = min(offset / total * 100.0, 100.0) if total else 0.0
                await safe_edit(app, chat_id, msg_id,
                    f"{TXT.ASCAN_HIST_PROGRESS_TITLE}\n{TXT.DIVIDER}\n"
                    f"{TXT.ASCAN_HIST_ROW_COMM} `{_comm.get('display_name') or _comm['url']}`\n"
    ...
    async def run_deepscan(chat_id: int, msg_id: int, tag: str):
        try:
            ...
            groups, offset = [], 0
            while True:
                res = await asyncio.to_thread(vk.groups.get, extended=1, count=1000, offset=offset)
                items = res.get('items', [])
                groups.extend(items)
                offset += 1000
                if offset >= res.get('count', 0): break
    ...
    async def handle_callbacks(_, cb: CallbackQuery):
        ...
        if cb.data.startswith("ascanwiz|"):
            ...
            if action == "cancel":
                ...
                _autoscan_wizard_sessions.pop(chat_id, None)
                await cb.answer()
                await safe_edit(app, chat_id, cb.message.id, TXT.ASCAN_STARTING, None)
