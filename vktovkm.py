...
class TXT:
    ...
    # ---- Failed item retry notice ----
    UPLOAD_FAILED_NOTICE = "⚠️ **PERMANENTLY FAILED** (out of retries)\n\n📼 `{title}`\n🛠 Stage: `{stage}`\n❌ Reason: `{reason}`"
    RETRY_ITEM_BTN        = "🔄 Retry"
    RETRY_QUEUED_TOAST    = "🔄 Re-queued — it'll pick back up shortly."
    RETRY_NOT_FOUND_TOAST = "⚠️ Already retried, removed, or not failed anymore."
    ...
    PAGE_NEXT                = "Next ▶"
    ASCAN_FULLSCAN_BTN       = "🔎"
    ASCAN_FULLSCAN_TOAST     = "🔎 Full scan started for #{tag} — check your chat for progress."
    ASCAN_FULLSCAN_GONE      = "⚠️ That hashtag no longer exists."
...
# Some VK failures are just "not propagated yet", not permanent. A freshly-posted
# video can come back "only available to followers/registered users" for a
# while and then quietly become downloadable hours later with no change on our
# end — Autoscan hits this constantly since it grabs videos within minutes of
# posting. Give these a long, escalating hold instead of burning through
# MAX_RETRIES in the first few minutes and giving up.
AVAILABILITY_ERROR_MARKERS = (
    "only available to followers",
    "only available for registered users",
    "only available for premium users",
    "video is unavailable",
)
AVAILABILITY_RETRY_DELAYS_SEC = [6 * 3600, 12 * 3600, 24 * 3600, 24 * 3600, 24 * 3600]  # ~3 days of patience
GENERIC_RETRY_DELAYS_SEC = [30, 120]  # 2 requeues then terminal — same attempt budget as the old MAX_RETRIES=3, just with a small gap instead of hammering instantly

def _is_availability_error(reason: str) -> bool:
    r = (reason or "").lower()
    return any(marker in r for marker in AVAILABILITY_ERROR_MARKERS)
...
# Per-tag locks guarding "resolve-or-create VK album" + playlist creation. Without
# this, two concurrent callers targeting the same tag (e.g. a manual /scanstart
# racing the Autoscan monitor loop, or a Force Rescan overlapping the periodic
# cycle) can both see "no matching album yet" and each call vk.video.addAlbum(),
# producing two identically-named VK albums for one tag.
_album_locks: dict[str, asyncio.Lock] = {}
_album_locks_guard = asyncio.Lock()

async def _get_album_lock(tag: str) -> asyncio.Lock:
    async with _album_locks_guard:
        lock = _album_locks.get(tag)
        if lock is None:
            lock = _album_locks[tag] = asyncio.Lock()
        return lock
...
def get_or_create_vk_album(vk, album_name: str) -> int:
    """Finds an existing album by title (case/whitespace-insensitive) or
    creates one. Paginates through the FULL album list — vk.video.getAlbums
    only returns 100 per call, and accounts with 100+ albums will otherwise
    never find albums past the first page, silently creating a duplicate
    every time that tag comes up again."""
    target = album_name.strip().lower()
    offset, count = 0, 100
    while True:
        res = vk.video.getAlbums(count=count, offset=offset)
        albums = res.get('items', [])
        if not albums: break
        for album in albums:
            if album.get('title', '').strip().lower() == target:
                return album['id']
        offset += count
...
class JobScheduler:
    ...
    def _init_db(self):
        ...
        self.conn.execute('''CREATE TABLE IF NOT EXISTS autoscan_seen (
            unique_id TEXT, tag TEXT, PRIMARY KEY (unique_id, tag)
        )''')
        self.conn.execute('''CREATE TABLE IF NOT EXISTS autoscan_state (
            key TEXT PRIMARY KEY, value TEXT
        )''')

        # Migrate older DBs where autoscan_seen had unique_id as a *sole* PK.
        # That schema let a video be recorded under only one tag ever, globally —
        # so a video posted with two tracked hashtags would only ever be caught
        # under whichever tag scanned first, and would then be permanently
        # invisible to the other tag. Widen the key to (unique_id, tag) so each
        # tag tracks its own sighting of a video independently.
        seen_cols = self.conn.execute("PRAGMA table_info(autoscan_seen)").fetchall()
        tag_is_key = any(c[1] == 'tag' and c[5] > 0 for c in seen_cols)
        if seen_cols and not tag_is_key:
            self.conn.execute('ALTER TABLE autoscan_seen RENAME TO autoscan_seen_old')
            self.conn.execute('''CREATE TABLE autoscan_seen (
                unique_id TEXT, tag TEXT, PRIMARY KEY (unique_id, tag)
            )''')
            self.conn.execute('INSERT OR IGNORE INTO autoscan_seen (unique_id, tag) SELECT unique_id, tag FROM autoscan_seen_old')
            self.conn.execute('DROP TABLE autoscan_seen_old')
            self.conn.commit()
        ...
        for stmt in (
            ...
            'ALTER TABLE playlist_items ADD COLUMN retry_after INTEGER DEFAULT 0',
        ):
    ...
    async def is_autoscan_seen(self, uid: str, tag: str) -> bool:
        async with self.lock:
            row = self.conn.execute('SELECT 1 FROM autoscan_seen WHERE unique_id = ? AND tag = ?', (uid, tag)).fetchone()
            return bool(row)
    ...
    async def find_reusable_playlist(self, caption: str) -> dict:
        """Finds an existing playlist targeting the same 'album_id|||tag' caption
        that isn't cancelled, so new matches for a tag extend it instead of
        spawning a duplicate playlist entry every scan cycle. A 'completed'
        playlist is fair game too — reopening it (see extend_playlist) is
        cheaper and less confusing than a second playlist for the same tag."""
        async with self.lock:
            row = self.conn.execute(
                'SELECT * FROM playlists WHERE caption = ? AND status IN ("active","paused","completed") ORDER BY rowid DESC LIMIT 1',
                (caption,)
            ).fetchone()
            return dict(row) if row else None

    async def extend_playlist(self, pl_id: str, added: int):
        async with self.lock:
            pl = self.conn.execute('SELECT * FROM playlists WHERE id = ?', (pl_id,)).fetchone()
            if not pl: return
            new_status = "active" if pl['status'] == "completed" else pl['status']
            self.conn.execute('UPDATE playlists SET total = ?, status = ? WHERE id = ?', (pl['total'] + added, new_status, pl_id))
            self.conn.commit()
    ...
    async def get_pending_items(self, pl_id: str, limit: int = 2) -> list[dict]:
        async with self.lock:
            return [dict(r) for r in self.conn.execute(
                'SELECT * FROM playlist_items WHERE playlist_id = ? AND status = "pending" AND retry_after <= ? ORDER BY rowid ASC LIMIT ?',
                (pl_id, int(time.time()), limit)
            ).fetchall()]

    async def get_pending_count(self, pl_id: str) -> int:
        async with self.lock:
            row = self.conn.execute('SELECT COUNT(*) FROM playlist_items WHERE playlist_id = ? AND status = "pending" AND retry_after <= ?', (pl_id, int(time.time()))).fetchone()
            return row[0] if row else 0

    async def get_global_pending_items(self, limit: int = 10) -> list[dict]:
        """Fetches the next N pending items across ALL active playlists that
        aren't currently on an availability hold (see retry_after / fail_or_retry)."""
        async with self.lock:
            return [dict(r) for r in self.conn.execute(
                '''SELECT pi.* FROM playlist_items pi
                   JOIN playlists p ON pi.playlist_id = p.id
                   WHERE pi.status = "pending" AND p.status = "active" AND pi.retry_after <= ?
                   ORDER BY pi.rowid ASC LIMIT ?''', (int(time.time()), limit)
            ).fetchall()]
    ...
    async def fail_or_retry(self, job: dict, reason: str) -> bool:
        """Returns True if this was the terminal failure (retries exhausted,
        item now sits in 'failed' state), False if it was silently re-queued
        (possibly on a long availability hold — see AVAILABILITY_ERROR_MARKERS)."""
        jid, item_id = job['id'], job.get('item_id') or job['id']
        retries = int(job.get('retries') or 0) + 1
        availability_hold = _is_availability_error(reason)
        delays = AVAILABILITY_RETRY_DELAYS_SEC if availability_hold else GENERIC_RETRY_DELAYS_SEC
        self.log_trace(jid, f"FAILURE (attempt {retries}/{len(delays) + 1}{' — availability hold, will re-check later' if availability_hold else ''}): {reason}")

        async with self.lock:
            if retries <= len(delays):
                retry_after = int(time.time()) + delays[retries - 1]
                self.conn.execute('UPDATE playlist_items SET status = "pending", retries = ?, retry_after = ? WHERE id = ?', (retries, retry_after, item_id))
    ...
    async def retry_failed_item(self, item_id: str) -> dict:
        """Resets a permanently-failed playlist item back to 'pending' so the
        drip-feed scheduler picks it up again on its next pass. Un-completes
        the parent playlist if needed. Returns the item row, or {} if the
        item doesn't exist or isn't currently 'failed'."""
        async with self.lock:
            row = self.conn.execute('SELECT * FROM playlist_items WHERE id = ? AND status = "failed"', (item_id,)).fetchone()
            if not row:
                return {}
            item = dict(row)
            self.conn.execute('UPDATE playlist_items SET status = "pending", retries = 0, retry_after = 0 WHERE id = ?', (item_id,))
            pl = self.conn.execute('SELECT * FROM playlists WHERE id = ?', (item['playlist_id'],)).fetchone()
            if pl:
                new_downloaded = max(0, pl['downloaded'] - 1)
                new_status = "active" if pl['status'] == "completed" else pl['status']
                self.conn.execute('UPDATE playlists SET downloaded = ?, status = ? WHERE id = ?', (new_downloaded, new_status, pl['id']))
            self.conn.commit()
            return item
...
class UploaderEngine:
    ...
    async def execute(self, job: dict):
        ...
        if not stage.startswith("uploaded"):
            ...
            target_album_id, album_title = parse_album_caption(pl.get('caption')) if pl else (None, "Default")
            ...
            try:
                upload_data = await asyncio.to_thread(get_upload_server, target_album_id)
            except Exception as e:
                # album_id can go stale if the target album was deleted/merged in VK
                # after this item was queued (e.g. while cleaning up a duplicate album).
                # VK's error for a dead album_id is generic ("[100] One of the parameters
                # specified was missing or invalid"), not a distinct "album not found" —
                # so on that specific code, with an album_id actually in play, re-resolve
                # or recreate the album by title and retry once before giving up.
                code = getattr(e, 'code', None) or getattr(e, 'error_code', None)
                if code is None:
                    m = re.search(r'\[(\d+)\]', str(e))
                    if m: code = int(m.group(1))
                if target_album_id and code == 100:
                    def re_resolve():
                        vk = get_vk_api()
                        return get_or_create_vk_album(vk, album_title)
                    new_album_id = await asyncio.to_thread(re_resolve)
                    if new_album_id != target_album_id:
                        await self.db.update_playlist(pl['id'], caption=f"{new_album_id}|||{album_title}")
                    upload_data = await asyncio.to_thread(get_upload_server, new_album_id)
                else:
                    raise
    ...
    if not page_tags:
        ...
        for t in page_tags:
            kb.append([
                ...
                InlineKeyboardButton(TXT.ASCAN_FULLSCAN_BTN, callback_data=f"ascan|fullscan|{t['id']}"),
...
async def queue_tagged_videos(db: JobScheduler, chat_id: int, tag: str, videos: list[dict], source_label: str = None) -> tuple[int, int]:
    ...
    # Serialized per-tag: two concurrent callers for the same tag (e.g. /scanstart
    # racing the Autoscan monitor) must not both resolve "no album yet" and each
    # create one — see _get_album_lock for the full story.
    lock = await _get_album_lock(tag)
    async with lock:
        target_album_id, missing = await asyncio.to_thread(dedupe_and_prepare)
        if not missing: return 0, len(videos)
        caption = f"{target_album_id}|||{tag}"
        existing_pl = await db.find_reusable_playlist(caption)
        if existing_pl:
            pl_id = existing_pl['id']
            await db.extend_playlist(pl_id, len(missing))
        else:
            pl_id = str(uuid.uuid4())[:8]
            await db.create_playlist(pl_id, source_label or f"deepscan:#{tag}", caption, len(missing), chat_id)
...
async def _autoscan_full_scan_community(db: JobScheduler, vk, comm: dict, tag_rows: list[dict], progress_cb=None) -> int:
    ...
    seen_pairs = set()  # (uid, tag) — dedupes reposts within this pass, per tag, so a
                         # video carrying multiple tracked hashtags matches each of them
...
async def _autoscan_light_scan_community(db: JobScheduler, app: Client, vk, comm: dict, tag_rows: list[dict]) -> int:
    ...
    patterns = {t['tag']: _hashtag_pattern(t['tag']) for t in tag_rows}
    matched_by_tag: dict[str, list[dict]] = {t['tag']: [] for t in tag_rows}
    seen_pairs = set()  # (uid, tag) — same reasoning as the full scan above
        ...
        for tag, pattern in patterns.items():
            if any(pattern.search(t) for t in texts):
                for v in post_videos:
                    ...
                    if key in seen_pairs: continue
                    seen_pairs.add(key)
                    if await db.is_autoscan_seen(uid, tag): continue
                    matched_by_tag[tag].append({'url': f"https://vk.com/video{uid}", 'title': v.get('title', 'VK Video'), 'unique_id': uid})
...
async def autoscan_cycle(db: JobScheduler, app: Client):
    ...
    if autoscan_cycle._running:
        log.info("Autoscan: cycle already in progress, skipping overlap")
        return
    autoscan_cycle._running = True
    try:
        tags = await db.get_autoscan_tags()
...
def setup_router(app: Client, db: JobScheduler, dl_q: asyncio.Queue, enc_q: asyncio.Queue, up_q: asyncio.Queue, dl_pool: "WorkerPool", enc_pool: "WorkerPool", up_pool: "WorkerPool"):
    ...
    async def catch_album_target(_, msg: Message):
        ...
        try:
            lock = await _get_album_lock(album_name.strip().lower())
            async with lock:
                ...
                caption = f"{target_album_id}|||{album_name}"
                existing_pl = await db.find_reusable_playlist(caption)
                if existing_pl:
                    pl_id = existing_pl['id']
                    await db.extend_playlist(pl_id, len(missing_videos))
                else:
                    pl_id = str(uuid.uuid4())[:8]
                    await db.create_playlist(pl_id, session['url'], caption, len(missing_videos), msg.chat.id)
    ...
    # ──────────────────────────────────────────────
# ──────────────────────────────────────────────
    # 11.3 — /scan & /scanstart #Hashtag (wall search by tag)
    # ──────────────────────────────────────────────
    async def run_hashtag_scan(chat_id: int, tag: str, url: str, auto_confirm: bool = False):
        ...
        try:
            ...
            # --- THE AUTOCONFIRM BYPASS ---
            if auto_confirm:
                await safe_edit(app, chat_id, m.id, TXT.HSCAN_RESOLVING_ALBUM)
                try:
                    queued, skipped = await queue_tagged_videos(db, chat_id, tag, videos, source_label=url)
                    # /scanstart implies "track this tag going forward" — add it to
                    # Autoscan once the batch is done so future posts get caught automatically.
                    tag_added = await db.add_autoscan_tag(tag)
                    # Mark every matched video as already-seen for this tag *now*, not
                    # after it's uploaded. queue_tagged_videos only dedupes against what's
                    # already landed in the VK album, so without this, an Autoscan monitor
                    # pass that fires while this batch is still downloading/uploading would
                    # see the same posts as "new" and re-queue them — duplicate downloads.
                    for v in videos:
                        await db.mark_autoscan_seen(v['unique_id'], tag)
                    autoscan_note = f"\n\n{TXT.ASCAN_TAG_ADDED.format(tag=tag)}" if tag_added else ""
                    if queued == 0:
                        await safe_edit(app, chat_id, m.id, TXT.HSCAN_ALL_PRESENT.format(count=len(videos), album=tag) + autoscan_note)
                    else:
                        await safe_edit(app, chat_id, m.id,
                            ...
                            f"{TXT.HSCAN_ROW_QUEUED} `{queued}`" + autoscan_note)
                    await refresh_dashboard_if_open()
                except Exception as e:
                    await safe_edit(app, chat_id, m.id, TXT.HSCAN_QUEUE_ERROR.format(err=e))
            else:
                # Normal behavior: show confirmation buttons
    ...
    @app.on_message(filters.regex(r"^/(scan|scanstart)\s+(#[^\s]+.*)") & filters.user(OWNER_ID))
    async def cmd_scan_hashtag(_, msg: Message):
        match = re.search(r"^/(scan|scanstart)\s+(#[^\s]+.*)", msg.text)
        command_used = match.group(1).lower()
        auto_confirm = (command_used == "scanstart")
        ...
        if urls:
            ...
            for tag in tags:
                asyncio.create_task(run_hashtag_scan(msg.chat.id, tag, url, auto_confirm=auto_confirm))
            return

        if msg.chat.id not in _hashtag_scan_pending:
            _hashtag_scan_pending[msg.chat.id] = []
            
        _hashtag_scan_pending[msg.chat.id].append({'tags': tags, 'stage': 'awaiting_link', 'auto_confirm': auto_confirm})
    ...
    async def intercept_urls_for_hashtags(_, msg: Message):
        ...
        url = msg.text.strip()
        auto_confirm = session.get('auto_confirm', False)
    ...
    async def auto_catch_playlist(_, msg: Message):
        ...
        try:
            lock = await _get_album_lock(target_album_name.strip().lower())
            async with lock:
                ...
                caption = f"{target_album_id}|||{target_album_name}"
                existing_pl = await db.find_reusable_playlist(caption)
                if existing_pl:
                    pl_id = existing_pl['id']
                    await db.extend_playlist(pl_id, len(items_to_upload))
                else:
                    pl_id = str(uuid.uuid4())[:8]
                    await db.create_playlist(pl_id, url, caption, len(items_to_upload), msg.chat.id)
    ...
    async def handle_callbacks(_, cb: CallbackQuery):
        ...
        # --- Failed Item Retry Callback ---
        if cb.data.startswith("retry_item|"):
            item_id = cb.data.split("|", 1)[1]
            item = await db.retry_failed_item(item_id)
            if not item:
                return await cb.answer(TXT.RETRY_NOT_FOUND_TOAST, show_alert=True)
            await cb.answer(TXT.RETRY_QUEUED_TOAST)
            try:
        ...
        if cb.data.startswith("ascan|"):
            ...
            if action == "addtag":
                ...
                row = next((t for t in rows if t['id'] == tag_id), None)
                if not row:
                    return await cb.answer(TXT.ASCAN_FULLSCAN_GONE, show_alert=True)
                tag = row['tag']
                await cb.answer(TXT.ASCAN_FULLSCAN_TOAST.format(tag=tag), show_alert=True)
                m = await app.send_message(chat_id, TXT.DSCAN_FETCHING_GROUPS)
                asyncio.create_task(run_deepscan(chat_id, m.id, tag))
                return
...
async def notify_permanent_failure(app: Client, job: dict, stage: str, reason: str):
    """Sent once an item exhausts MAX_RETRIES and is parked as 'failed'.
    Carries a Retry button that requeues just that item."""
    item_id = job.get('item_id') or job.get('id')
    chat_id = job.get('chat_id') or OWNER_ID
    title = clean_title(job.get('title') or item_id or "Unknown")[:60]
    text = TXT.UPLOAD_FAILED_NOTICE.format(title=title, stage=stage, reason=str(reason)[:250])
    kb = InlineKeyboardMarkup([[InlineKeyboardButton(TXT.RETRY_ITEM_BTN, callback_data=f"retry_item|{item_id}")]])
    try:
        await app.send_message(chat_id, text, reply_markup=kb)
    except Exception as e:
        log.error(f"Failed to send failure notice for item {item_id}: {e}")
            ...
            try:
                ...
                if j_data:
                    try:
                        ...
                        if await db.fail_or_retry(j_data, str(e)):
                            asyncio.create_task(notify_permanent_failure(app, j_data, "download", str(e)))
            except Exception: pass
            ...
            try:
                ...
                if j_data:
                    try:
                        ...
                        if await db.fail_or_retry(j_data, str(e)):
                            asyncio.create_task(notify_permanent_failure(app, j_data, "encode", str(e)))
            except Exception: pass
                ...
                if j_data:
                    try:
                        ...
                        if latest and (latest.get('stage') or "").lower().startswith("uploaded"):
                            ...
                            if await db.fail_or_retry(j_data, str(e)):
                                asyncio.create_task(notify_permanent_failure(app, j_data, "upload", str(e)))
            except Exception: pass
