...
async def run_custom_transfer(user_chat_id, state, mode, custom_tags):
    ...
    async for msg in user_app.get_chat_history(target_chat_id):
        ...
        if mode == "ALL":
            found_tag = None
            for am in album_msgs:
                extracted = extract_first_tag(am.caption or am.text or "")
                if extracted:
                    found_tag = extracted
                    break
            ...
            if not found_tag:
                ...
                found_tag = fallback_tag
                
            for am in album_msgs:
                results.setdefault(f"#{found_tag}", []).append(am)

        elif mode == "TAGS":
            matched = parse_master_caption_bundle(album_msgs, target_tags_lower)
            for m in matched:
    ...
    for tag, msgs in results.items():
        ...
        for m in sorted(msgs, key=lambda x: x.id):
            try:
                if await tg_execute_message_copy(target_chat_id, m, master_forum_id, topic_id, tag, delete_originals):
                    copied += 1
            except Exception as e:
...
async def live_monitor_handler(client, message):
    ...
    if autotransfer_row and autotransfer_row[0] == "ACTIVE":
        ...
        if mode == "ALL":
            found_tag = extract_first_tag(message.caption or message.text or "")
            if not found_tag:
