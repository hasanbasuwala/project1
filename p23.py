...
async def _in_stash_state(_, __, message):
    return bool(message.from_user and message.from_user.id in STASH_ROUTE_STATE)

stash_state_filter = filters.create(_in_stash_state)
...
@bot_app.on_message(filters.text & stash_state_filter & ~filters.command(["cancel", "start"]))
...
@bot_app.on_message(filters.private & filters.text & ~filters.regex(r"^/"))
