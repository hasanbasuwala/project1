...
FEATURES & ARCHITECTURE:
  ...
  • Multi-Playlist Scheduler: PAUSE ALL / RESUME ALL, graceful across every
    active playlist at once, in addition to the existing per-playlist toggle
...
CRASH-RECOVERY MODEL (v2):
  ...
  • PAUSE ALL / RESUME ALL flips every playlist's status in a single atomic
    UPDATE. In-flight jobs are never touched -- they finish naturally, same
    guarantee as the existing per-playlist pause. Because the drip-feed loop
    and reconcile_on_startup() both key off each playlist's own `status`
    column (never a global flag), a bulk pause is indistinguishable, to the
    rest of the pipeline, from someone having paused every playlist by hand
    -- including across a crash/reboot mid-pause.
───────────────────────────────────────────────────────────────
"""
...
class JobScheduler:
    ...
    async def pause_all_playlists(self) -> int:
        """
        Multi-playlist scheduler: pauses every currently-active playlist in
        one atomic UPDATE (not a Python loop over update_playlist calls,
        which would let the drip-feed loop interleave a claim between two
        individual writes). In-flight jobs on every playlist are left
        completely alone -- exactly the same guarantee as a single-playlist
        pause -- they finish naturally instead of being killed mid-transfer.
        Returns how many playlists actually changed state.
        """
        async with self.lock:
            cur = self.conn.execute(
                'UPDATE playlists SET status = "paused" WHERE status = "active"'
            )
            self.conn.commit()
            return cur.rowcount
    ...
    async def resume_all_playlists(self) -> int:
        """
        Counterpart to pause_all_playlists(): flips every currently-paused
        playlist back to 'active' in one atomic UPDATE. The drip-feed loop's
        existing held-job release logic (keyed per-playlist) then picks each
        one up on its next 3s tick and requeues its held jobs -- no separate
        code path needed for "resume all" vs. "resume one".
        """
        async with self.lock:
            cur = self.conn.execute(
                'UPDATE playlists SET status = "active" WHERE status = "paused"'
            )
            self.conn.commit()
            return cur.rowcount
    ...
    async def reconcile_on_startup(self) -> dict:
        ...
        This applies identically whether a playlist was paused individually
        or as part of a PAUSE ALL -- both just set status="paused", so
        recovery can't tell (and doesn't need to) which one happened.
...
async def playlist_drip_feed_loop(db: JobScheduler, dl_q: asyncio.Queue, enc_q: asyncio.Queue, up_q: asyncio.Queue,
    ...
    Multi-playlist scheduler note: this loop already iterates every active
    playlist and skips any whose status is 'paused' -- so PAUSE ALL / RESUME
    ALL need no special-casing here at all. Setting every playlist's status
    to 'paused' in one DB transaction (JobScheduler.pause_all_playlists)
    makes the very next tick of this loop behave exactly as if each
    playlist had been paused individually, one at a time."""
    while True:
        ...
        try:
            ...
            # a reboot, and ones released by RESUME ALL. This runs
            # independent of the disk/concurrency gate below since these
            # are already-existing jobs, not new claims.
            ...
            # TRUE round-robin: claim at most ONE pending item per playlist
            # per pass, then loop back to the start of the list and do
            # another pass. This is what makes 2+ playlists progress
            # visibly together from the very first tick, instead of one
            # playlist draining the whole shared slot budget before the
            # next playlist gets anything (the old priority-order behavior).
            # A pass that claims nothing from any playlist (everyone's
            # pending queue is empty) stops the loop instead of spinning.
            eligible_playlists = [
                pl for pl in active_playlists
                if pl['status'] not in ('paused', 'cancelled', 'completed')
            ]
            made_progress_this_pass = True
            while global_slots_free > 0 and eligible_playlists and made_progress_this_pass:
                ...
                for pl in eligible_playlists:
                    ...
                    if not pending_items:
                        continue  # this playlist is out of pending items for now
...
async def probe_codecs(path: Path) -> tuple[str | None, str | None]:
    """Returns (video_codec, audio_codec) via ffprobe, e.g. ('h264','aac').
    audio_codec is None if there's no audio stream at all."""
    async def _probe(stream_selector: str) -> str | None:
        try:
            cmd = [
    ...
    return await _probe("v:0"), await _probe("a:0")


def has_faststart(path: Path) -> bool:
    """
    Lightweight top-level MP4 box scan: True if the 'moov' atom appears
    before 'mdat' (i.e. the file is already streaming-optimized), without
    needing to touch or re-encode anything. False (or a scan failure)
    just means we fall back to the normal remux/encode path -- never a
        ...
        with open(path, "rb") as f:
            ...
            while offset < file_size:
                ...
                if size == 1:  # 64-bit extended size follows
                    ...
                    break  # box extends to EOF -- nothing meaningful follows
                offset += size
                f.seek(offset)
    except Exception:
        pass
...
class EncoderEngine:
    async def execute(self, job: dict, db: JobScheduler):
        ...
        if dst.exists() and dst.stat().st_size > 0:
            ...
            vcodec, acodec = await probe_codecs(src)
            telegram_safe_codecs = (
                src.suffix.lower() == ".mp4"
                and vcodec == "h264"
                and acodec in (None, "aac")
            )

            if telegram_safe_codecs and has_faststart(src):
                # Already H.264 + AAC (or no audio) in an MP4 container, and
                # already streaming-optimized -- there is nothing for ffmpeg
                # to usefully do here. Re-muxing would just burn CPU/time and
                # (for the audio track) needlessly re-encode it lossily for
                # no benefit. Move it straight into place instead.
                db.log_trace(jid, f"Source already Telegram-ready (v={vcodec}, a={acodec}, faststart present) — skipping ffmpeg entirely.")
                src.rename(dst)  # same filesystem -- atomic, no data duplication
            else:
                if telegram_safe_codecs:
                    # Codecs are already fine, it just needs the moov atom
                    # moved to the front -- a cheap copy-only remux, no
                    # re-encoding of either stream.
                    db.log_trace(jid, f"Source codecs already Telegram-safe (v={vcodec}, a={acodec}) but not faststart — remuxing only, no re-encode.")
                    ffmpeg_args = ["ffmpeg", "-y", "-i", str(src), "-c", "copy", "-movflags", "+faststart", "-f", "mp4", str(tmp_dst)]
                else:
                    db.log_trace(jid, f"Source codecs not Telegram-safe (v={vcodec}, a={acodec}) — transcoding audio to AAC, copying video.")
                    ffmpeg_args = [
                        "ffmpeg", "-y", "-i", str(src),
...
def render_worker_panel(dl_pool: "WorkerPool", enc_pool: "WorkerPool", up_pool: "WorkerPool") -> tuple[str, InlineKeyboardMarkup]:
    text = (
        ...
        f"`⚙️ ENCODE   : {enc_pool.current_count()}/{enc_pool.target} active`\n"
        f"`📤 UPLOAD   : {up_pool.current_count()}/{up_pool.target} active`\n"
        "`━━━━━━━━━━━━━━━━━━━━━━━━━━`\n"
        "Tap − / + to resize. Growing takes effect immediately; shrinking "
        "waits for the current job to finish (nothing gets killed mid-transfer)."
    )
    kb = [
        [
            InlineKeyboardButton("−", callback_data="wk|dl|-1"),
            InlineKeyboardButton(f"📥 DL: {dl_pool.target}", callback_data="noop"),
            InlineKeyboardButton("+", callback_data="wk|dl|1"),
        ],
        [
            InlineKeyboardButton("−", callback_data="wk|enc|-1"),
            InlineKeyboardButton(f"⚙️ ENC: {enc_pool.target}", callback_data="noop"),
            InlineKeyboardButton("+", callback_data="wk|enc|1"),
        ],
        [
            InlineKeyboardButton("−", callback_data="wk|up|-1"),
            InlineKeyboardButton(f"📤 UP: {up_pool.target}", callback_data="noop"),
            InlineKeyboardButton("+", callback_data="wk|up|1"),
        ],
...
async def render_dashboard(db: JobScheduler, tab: str = "playlists", exp_pl: str = None, exp_bucket: str = None, exp_jid: str = None) -> tuple[str, InlineKeyboardMarkup]:
    ...
    # ── Multi-playlist scheduler: global pause/resume toggle ──
    # Shown whenever there's at least one active/paused playlist. Reflects
    # the aggregate state: if ANY playlist is still active, offer PAUSE ALL;
    # only once every playlist is paused does it flip to RESUME ALL. This
    # mirrors the per-playlist button but acts on every row in one shot.
    if playlists:
        any_active = any(p['status'] == 'active' for p in playlists)
        if any_active:
            kb.append([InlineKeyboardButton("⏸ PAUSE ALL PLAYLISTS", callback_data="pause_all")])
        else:
            kb.append([InlineKeyboardButton("▶️ RESUME ALL PLAYLISTS", callback_data="resume_all")])
...
def setup_router(app: Client, db: JobScheduler, dl_q: asyncio.Queue, enc_q: asyncio.Queue, up_q: asyncio.Queue,
    ...
    async def cmd_pause_all(_, msg: Message):
        """Text-command shortcut for the same PAUSE ALL action as the
        dashboard button, for when you're not looking at the dashboard."""
        count = await db.pause_all_playlists()
        await msg.reply(
            f"⏸ Paused {count} playlist(s). In-progress jobs finish naturally; "
            "nothing new starts (including after a crash/reboot) until resumed."
        )
        global _dash_msg_id, _dash_chat_id, _dash_tab, _expanded_pl, _expanded_bucket, _expanded_jid
        if _dash_msg_id and _dash_chat_id:
            text, kb = await render_dashboard(db, _dash_tab, _expanded_pl, _expanded_bucket, _expanded_jid)
    ...
    async def cmd_resume_all(_, msg: Message):
        """Text-command shortcut for the same RESUME ALL action as the
        dashboard button."""
        count = await db.resume_all_playlists()
        await msg.reply(f"▶️ Resumed {count} playlist(s). Held jobs release within a few seconds.")
        global _dash_msg_id, _dash_chat_id, _dash_tab, _expanded_pl, _expanded_bucket, _expanded_jid
        if _dash_msg_id and _dash_chat_id:
            text, kb = await render_dashboard(db, _dash_tab, _expanded_pl, _expanded_bucket, _expanded_jid)
            await safe_edit(app, _dash_chat_id, _dash_msg_id, text, kb)
    ...
    async def cmd_workers(_, msg: Message):
        ...
        if not changes:
            return await msg.reply("Couldn't parse that. Usage: `/vk_workers dl=5 enc=3 up=2`, or just `/vk_workers` for the tappable panel.")
    ...
    async def handle_callbacks(_, cb: CallbackQuery):
        ...
        if cb.data.startswith("wk|"):
            ...
            delta = int(delta_str)
            before = pool.target
            new_target = max(0, before + delta)
            await pool.adjust(new_target)

            note = "" if new_target >= before else " (takes effect as the current job finishes)"
            await cb.answer(f"{pool_key.upper()} workers: {before} → {new_target}{note}")
        ...
        if cb.data.startswith("delmsg|"):
            ...
            # Multi-playlist scheduler: graceful pause across every active
            # playlist at once. In-flight jobs on all of them keep running
            # to completion -- only new claims stop, exactly like a single
            # playlist pause, just applied everywhere simultaneously.
            count = await db.pause_all_playlists()
            await cb.answer(
                f"⏸ ALL PLAYLISTS PAUSED ({count})\n\n"
                "In-progress jobs on every playlist finish naturally, not killed. "
                "No new videos start anywhere -- including after a crash/reboot -- "
                "until you hit Resume All.",
                show_alert=True
            )
            ...
            count = await db.resume_all_playlists()
            await cb.answer(
                f"▶️ ALL PLAYLISTS RESUMED ({count}).\n\nHeld jobs release within a few seconds.",
                show_alert=True
            )
