...
def setup_router(app: Client, db: JobScheduler, dl_q: asyncio.Queue, enc_q: asyncio.Queue, up_q: asyncio.Queue, dl_pool: "WorkerPool", enc_pool: "WorkerPool", up_pool: "WorkerPool"):
    ...
    async def cmd_scan(_, msg: Message):
        ...
        def perform_scan():
            ...
            # 1. Normalize the URL (fixes mobile links and vk.ru)
            ...
            if comm_match: 
                comm_id = -int(comm_match.group(1))
            
            # B. If you accidentally pasted a video link (e.g., video-123456_7890)
            elif 'video-' in clean_url:
                vid_match = re.search(r'video(-?\d+)_', clean_url)
                if vid_match:
                    # Community IDs in video links are already negative
                    comm_id = int(vid_match.group(1))
            
            # C. Match custom screen names (e.g., vk.com/my_community or vk.com/@my_community)
            else:
                ...
                if screen_match:
                    ...
                    try:
                        ...
                        # Types can be 'group', 'page', or 'event'
                        if res and res.get('type') in ('group', 'page', 'event'): 
                            comm_id = -int(res['object_id'])
                        else:
                            print(f"VK API returned non-community type for {screen_name}: {res}")
                    except Exception as e: 
                        print(f"Resolve failed for {screen_name}: {e}")
            
            if not comm_id: raise ValueError(f"Could not resolve community ID from: {clean_url}")

            # ... [Keep the rest of the perform_scan function exactly the same starting from `videos, wall = [], []`]
    ...
    async def catch_album_target(_, msg: Message):
        ...
        try:
            ...
            # Re-render scan UI (clear prompt state)
            text, kb = render_scan_ui(msg.chat.id)
    ...
    @app.on_message(filters.regex(r"https?://") & filters.user(OWNER_ID) & ~filters.command(["scan", "vk_dash", "vk_workers", "vk_pause_all", "vk_resume_all"]))
